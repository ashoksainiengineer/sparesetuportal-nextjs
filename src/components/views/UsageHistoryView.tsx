  ...
  return (
    <div className={`animate-fade-in space-y-6 pb-20 font-roboto font-bold uppercase tracking-tight transition-all duration-500 ${loading ? 'opacity-60 blur-[1px] pointer-events-none' : 'opacity-100 blur-0'}`}>
      <div className="bg-white p-6 rounded-xl border shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center border-l-4 border-red-600 gap-4 uppercase font-black">
        ...
        <div className="flex gap-2 w-full md:w-auto font-black uppercase"><button onClick={fetchLogs} disabled={loading} className="bg-indigo-600 text-white px-5 py-2.5 rounded-xl text-[10px] font-black shadow-md flex items-center justify-center gap-2 hover:bg-indigo-700 transition-all uppercase tracking-widest font-black uppercase"><i className={`fa-solid fa-sync ${loading ? 'animate-spin' : ''} uppercase`}></i> Refresh</button><button onClick={exportCSV} className="bg-emerald-600 text-white px-5 py-2.5 rounded-xl text-[10px] font-black shadow-md flex items-center justify-center gap-2 hover:bg-emerald-700 transition-all uppercase font-black uppercase tracking-widest font-black uppercase"><i className="fa-solid fa-file-csv uppercase"></i> Export CSV</button></div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden font-black uppercase font-black uppercase">
        <div className="overflow-x-auto font-black uppercase font-black uppercase"><table className="w-full text-left tracking-tight font-black uppercase font-black uppercase"><thead className="bg-slate-50 text-slate-400 text-[10px] font-black border-b uppercase tracking-widest font-black uppercase font-black uppercase"><tr><th className="p-5 pl-8 font-black uppercase uppercase">Time Audit</th><th className="p-5 font-black uppercase uppercase">Material Details</th><th className="p-5 text-center font-black uppercase uppercase">Qty</th><th className="p-5 font-black uppercase uppercase">Job Purpose</th><th className="p-5 text-center font-black uppercase uppercase">Action</th></tr></thead><tbody className="divide-y text-[11px] font-bold uppercase font-black font-black uppercase">
