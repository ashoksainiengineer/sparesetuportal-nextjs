    ...
    <div className="animate-fade-in space-y-6 pb-20 font-roboto font-bold uppercase tracking-tight">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-left tracking-tight">
            <thead className="bg-slate-50 text-slate-400 text-[10px] font-black border-b uppercase tracking-widest"><tr><th className="p-5 pl-8">Time Audit</th><th className="p-5">Material Details</th><th className="p-5 text-center">Qty</th><th className="p-5">Job Purpose</th><th className="p-5 text-center">Action</th></tr></thead>
            <tbody className="divide-y text-[11px] font-bold">
              {loading ? <tr><td colSpan={5} className="p-20 text-center animate-pulse text-slate-300 font-black uppercase tracking-[0.2em]">Updating Feed...</td></tr>
              : currentLogs.length > 0 ? currentLogs.map((l: any) => (
                <tr key={l.id} className="hover:bg-slate-50 transition border-b uppercase">
                  ...
                  <td className="p-5"><div className="text-slate-600 italic lowercase font-medium bg-slate-50 p-2 rounded-lg border border-dashed border-slate-200 max-w-[200px] truncate">{l.purpose || '-- no note --'}</div></td>
                  <td className="p-5 text-center"><button onClick={() => deleteLog(l.id)} className="text-slate-300 hover:text-red-500 transition-all flex items-center justify-center mx-auto"><i className="fa-solid fa-trash-can text-sm"></i></button></td>
                </tr>)) : <tr><td colSpan={5} className="p-20 text-center text-slate-300 font-black uppercase tracking-widest">No Records Found</td></tr>}
            </tbody></table></div>
      </div>
    </div>
  );
