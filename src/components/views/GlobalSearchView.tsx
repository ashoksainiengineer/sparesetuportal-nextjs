  ...
  const handleSendRequest = async () => {
    ...
    try {
        const { data: liveRows, error: fetchErr } = await supabase
            .from("inventory")
            .select("qty")
            .eq("item", requestItem.item)
            .eq("spec", requestItem.spec)
            .eq("holder_uid", requestItem.holder_uid);

        if (fetchErr || !liveRows) throw new Error("Fetch failed");

        const totalLiveStock = liveRows.reduce((sum, row) => sum + Number(row.qty), 0);

        if (requestedQty > totalLiveStock) { 
            alert(`Stock Shortage! Only ${totalLiveStock} available in total with this engineer.`); 
            setSubmitting(false); 
            return; 
        }
        ...
        // --- UPDATED: Removed 'view_by_requester' as it's missing in your schema ---
        ...
        alert("System Issue: " + err.message); 
    } finally { 
        setSubmitting(false); 
    }
};
  ...
  <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[9999] flex items-center justify-center p-4">
    <div className="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden animate-scale-in border-t-8 border-indigo-600 uppercase font-bold">
      <div className="p-6 bg-slate-50 flex justify-between items-center border-b font-black">
        <div>
          <h3 className="text-slate-800 text-lg font-black">{bifurcateItem.item}</h3>
          <p className="text-[10px] text-slate-500 mt-1">SPEC: {bifurcateItem.spec}</p>
        </div>
        <button onClick={() => setBifurcateItem(null)} className="w-10 h-10 bg-white shadow-sm border rounded-full flex items-center justify-center text-slate-400 hover:text-red-500 transition-colors">
      ...
      <div className="p-6 overflow-y-auto max-h-[70vh] space-y-3">
        {Object.entries(
          bifurcateItem.occurrences
            .filter((o: any) => Number(o.qty) > 0)
            .reduce((acc: any, curr: any) => {
              if (!acc[curr.holder_unit]) acc[curr.holder_unit] = { total: 0, engineers: {} };
              acc[curr.holder_unit].total += Number(curr.qty);
              const engId = curr.holder_uid;
              if (!acc[curr.holder_unit].engineers[engId]) {
                acc[curr.holder_unit].engineers[engId] = { ...curr, sumQty: 0 };
              }
              acc[curr.holder_unit].engineers[engId].sumQty += Number(curr.qty);
              return acc;
            }, {})
        ).map(([zoneName, zoneData]: any) => (
          <div key={zoneName} className="border-2 border-slate-100 rounded-2xl overflow-hidden">
            <div 
              onClick={() => setExpandedZone(expandedZone === zoneName ? null : zoneName)} 
            ...
            >
              ...
              <div className="text-right flex items-center gap-4">
                <div>
                  ...
                  <p className={`text-[8px] font-bold ${expandedZone === zoneName ? 'text-white/50' : 'text-slate-400'}`}>ZONE TOTAL</p>
                </div>
                ...
                <table className="w-full text-left text-[10px] font-bold uppercase">
                  <thead className="text-slate-400 border-b tracking-widest text-[9px]">
                    <tr>
                      ...
                      <th className="p-3 text-center">Total Holding</th>
                      <th className="p-3 text-center">Action</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {Object.values(zoneData.engineers).map((eng: any, i: number) => (
                      <tr key={i} className="hover:bg-white transition-colors">
                        <td className="p-3 text-slate-700 font-black">
                          {eng.holder_name}
                          {eng.holder_uid === profile?.id && <span className="ml-2 bg-blue-100 text-blue-600 text-[7px] px-1.5 py-0.5 rounded border border-blue-200">YOU</span>}
                        </td>
                        <td className="p-3 text-center font-black text-slate-900 text-xs">
                          {eng.sumQty} {eng.unit}
                        </td>
                        <td className="p-3 text-center font-black">
                          {eng.holder_unit === profile?.unit ? (
                            <span className="text-[9px] text-slate-400 font-black uppercase italic">Internal Zone Item</span>
                          ) : (
                            <button 
                              onClick={(e) => { e.stopPropagation(); setRequestItem(eng); }} 
                              className="bg-[#ff6b00] text-white px-3 py-1 rounded-[4px] text-[9px] font-black tracking-widest shadow-md hover:bg-orange-600"
                            >
                              Request
                            </button>
                          )}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[10001] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 relative animate-scale-in">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border mb-6 text-xs">
              ...
              <p className="text-indigo-600 mt-1 uppercase font-bold">{requestItem.holder_unit} | Balance: {requestItem.sumQty || requestItem.qty} {requestItem.unit}</p>
            </div>
            <div className="space-y-4">
              ...
              <button disabled={submitting} onClick={handleSendRequest} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black tracking-widest uppercase hover:bg-black transition-all">
