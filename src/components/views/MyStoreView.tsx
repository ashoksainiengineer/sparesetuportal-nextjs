            ...
            {currentItems.length > 0 ? currentItems.map(i => (
              <tr key={i.id} className="hover:bg-slate-50 transition border-b uppercase">
                <td className="p-5 pl-8 leading-tight">
                  <div className="text-slate-800 font-bold text-[14px] flex items-center gap-2">{i.item}{i.is_manual && <span className="bg-orange-100 text-orange-600 text-[8px] px-1 rounded font-black border border-orange-200">M</span>}</div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-scale-in uppercase font-bold">
            <div className="p-6 border-b bg-slate-50 flex justify-between items-center"><h3 className="font-black text-slate-800 text-lg">{editItem ? 'Edit Existing Item' : 'Add New Material'}</h3><button onClick={resetForm}><i className="fa-solid fa-xmark text-slate-400"></i></button></div>
            <div className="p-6 space-y-4 font-bold">
              {editItem && !form.isManual && <div className="bg-blue-50 p-3 rounded-lg text-[9.5px] text-blue-700 flex items-center gap-2 border border-blue-100"><i className="fa-solid fa-lock text-sm"></i> Standard Catalog Item: Only Quantity & Unit are editable.</div>}
              {!editItem && (
                <div className="flex justify-between items-center bg-orange-50 p-3 rounded-xl border border-orange-100 mb-2">
                  <div className="flex flex-col"><span className="text-[10px] text-orange-800 font-black">Item not in Catalog list?</span><span className="text-[8px] text-orange-600 font-medium lowercase">enable manual entry to type details</span></div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={handleSaveItem} className="flex-[3] py-4 iocl-btn text-white rounded-2xl shadow-lg font-black uppercase tracking-[0.2em] text-sm">{editItem ? 'Confirm Changes' : 'Save Stock'}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {consumeItem && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-scale-in uppercase font-bold">
            <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-black text-slate-800">Consume Stock</h3><button onClick={() => { setConsumeItem(null); setConsumeForm({ qty: "", note: "" }); }}><i className="fa-solid fa-xmark text-slate-400"></i></button></div>
            ...
            <div className="space-y-4">
              ...
              <div><label className="text-[10px] text-slate-500 font-black uppercase mb-1 tracking-widest">Purpose / Note</label><textarea placeholder="Maintenance / Job Description..." className="w-full p-4 border-2 rounded-2xl font-bold h-24 focus:border-orange-500 outline-none text-xs" value={consumeForm.note} onChange={e => setConsumeForm({ ...consumeForm, note: e.target.value })}></textarea></div>
              <button onClick={handleConsume} className="w-full py-4 bg-slate-900 text-white rounded-2xl shadow-xl font-black tracking-widest hover:bg-black transition-all">Confirm Consumption</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showSummary && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-scale-in uppercase font-bold">
            <div className="p-6 border-b bg-indigo-50 flex justify-between items-center"><h3 className="font-black text-indigo-900 text-lg tracking-tight"><i className="fa-solid fa-boxes-stacked mr-2"></i> Local Store Summary</h3><button onClick={() => setShowSummary(false)}><i className="fa-solid fa-xmark text-slate-400"></i></button></div>
            ...
            <div className="p-4 bg-slate-50 text-center"><button onClick={() => setShowSummary(false)} className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] hover:text-indigo-600">Close Summary</button></div>
          </div>
        </div>
      )}
    </div>
