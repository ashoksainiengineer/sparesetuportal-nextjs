...
export default function MyStoreView({ profile, fetchProfile }: any) {
  ...
  // --- 1. ENGINE: AGGREGATION & SORTING (Point 4) ---
  ...
  // --- 2. FILTER ENGINE (Point 4: Out of Stock Filter) ---
  const filteredList = aggregatedList.filter((i: any) => {
    ...
    // Category filter including Out of Stock logic
    ...
    // Point 4: Zero qty items move to bottom
    if (a.totalQty === 0 && b.totalQty !== 0) return 1;
    if (a.totalQty !== 0 && b.totalQty === 0) return -1;
    return 0;
  });

  // Point 5: Count items with 0 qty
    ...
    <div className="animate-fade-in space-y-6 pb-20 font-roboto font-bold uppercase">
      ...
      <div className="bg-white p-6 rounded-xl border shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto">
            <button onClick={exportCSV} className="bg-emerald-600 text-white px-4 py-3 rounded-xl text-[10px] font-black shadow-md flex items-center justify-center gap-2 flex-1 md:flex-none uppercase tracking-widest"><i className="fa-solid fa-file-csv"></i> Export Sheet</button>
      ...
      {outOfStockCount > 0 && (
        <div className="bg-orange-50 border-l-4 border-orange-500 p-4 rounded-lg flex items-center gap-3 animate-pulse shadow-sm">
            ...
            <div>
                <p className="text-orange-900 text-xs font-black uppercase tracking-widest">Action Required: {outOfStockCount} items are completely out of stock</p>
                <p className="text-orange-700 text-[10px] font-bold lowercase">use '!!! OUT OF STOCK !!!' filter in category to identify items</p>
            </div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-4 bg-white p-4 rounded-xl border shadow-sm">
        <div className="relative w-full">
            ...
            <input type="text" placeholder="Search Materials..." className="w-full pl-10 pr-4 py-3 border-2 border-slate-100 rounded-xl text-xs outline-none focus:border-orange-400 font-bold uppercase shadow-inner transition-all" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
      ...
      {/* Main Table (Point 6: Header rename) */}
                        ...
                        {bifurcationItem.records.map((r: any) => (
                            <tr key={r.id} className="hover:bg-slate-50">
                                <td className="p-4 pl-6 leading-tight">
                                    ...
                                    <p className="text-[8px] text-slate-400 lowercase italic">full desc: {r.item}</p>
                                </td>
      ...
      {/* MODAL: ADD MATERIAL (Point 2: Label update) */}
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[10000] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-scale-in uppercase font-bold">
            <div className="p-6 border-b bg-slate-50 flex justify-between items-center"><h3 className="font-black text-slate-800 text-lg uppercase tracking-tight">{editItem ? 'Edit Material' : 'Add Stock'}</h3><button onClick={resetForm} className="text-slate-400 hover:text-red-500 transition-colors"><i className="fa-solid fa-xmark text-xl"></i></button></div>
            <div className="p-6 space-y-4 font-bold">
              {!editItem && (
                <div className="flex justify-between items-center bg-orange-50 p-4 rounded-xl border-2 border-dashed border-orange-200 mb-2 shadow-inner">
                  <div className="flex flex-col"><span className="text-[10px] text-orange-800 font-black tracking-tight uppercase">ITEM NOT IN LIST?</span><span className="text-[8.5px] text-orange-600 font-medium lowercase">toggle manual mode to enter details</span></div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button onClick={handleSaveItem} className="flex-[3] py-4 iocl-btn text-white rounded-2xl shadow-lg font-black uppercase tracking-[0.2em] text-sm hover:opacity-90 transition-all">Confirm</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* MODAL: CONSUME MATERIAL (Point 1: Close button added) */}
      {consumeItem && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[10001] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-scale-in uppercase font-bold relative">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-6 shadow-inner">
              <p className="text-[10px] text-slate-400 font-black mb-1 uppercase tracking-widest">Selected Item</p>
          ...
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-scale-in uppercase font-bold">
            ...
            <div className="p-6 max-h-[60vh] overflow-y-auto">
              <table className="w-full text-left text-xs font-bold uppercase">
                <thead className="border-b text-slate-400 uppercase tracking-widest"><tr><th className="pb-3">Category &gt; Sub-Category</th><th className="pb-3 text-right">Aggregated Total</th></tr></thead>
