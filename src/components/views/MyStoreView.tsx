...
export default function MyStoreView({ profile, fetchProfile }: any) {
  ...
  const [bifurcationItem, setBifurcationItem] = useState<any>(null); // State for split-up view
  ...
  // --- 1. LOGIC: AGGREGATION (Grouping same items in the same zone) ---
  const groupedItems = myItems.reduce((acc: any, item: any) => {
    const key = `${item.item}-${item.spec}`; // Grouping key
    if (!acc[key]) {
      acc[key] = { 
        ...item, 
        totalQty: 0, 
    ...
    acc[key].records.push(item); // Storing individual records for bifurcation
    return acc;
  }, {});
  ...
  // Filter Logic (Same as Global Search)
  ...
  const handleSaveItem = async () => {
    ...
    try {
      ...
      resetForm(); fetchStore(); fetchProfile(); alert("Inventory Updated!");
  ...
  const handleConsume = async () => {
    ...
    try {
      ...
      setConsumeItem(null); setConsumeForm({ qty: "", note: "" }); fetchStore(); alert("Material Consumed!");
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-20 font-roboto font-bold uppercase">
      ...
      <div className="bg-white p-6 rounded-xl border shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
        <div><h2 className="text-xl font-black text-slate-800 uppercase tracking-widest leading-none">My Local Store</h2><p className="text-[10px] font-black bg-blue-50 text-blue-700 px-2 py-0.5 rounded mt-2 inline-block">ZONE: {profile?.unit}</p></div>
        <div className="flex gap-2 w-full md:w-auto">
            <button onClick={() => setShowSummary(true)} className="bg-indigo-600 text-white px-4 py-3 rounded-xl text-[10px] font-black shadow-md flex items-center justify-center gap-2 flex-1 md:flex-none uppercase tracking-widest transition-all hover:bg-indigo-700"><i className="fa-solid fa-chart-bar"></i> Stock Summary</button>
      ...
      {/* Expanded Search & Reordered Filters */}
      <div className="flex flex-col gap-4 bg-white p-4 rounded-xl border shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <select disabled={selCat === "all"} className="border-2 border-slate-50 rounded-lg text-[10px] p-3 bg-white font-bold uppercase shadow-sm cursor-pointer" value={selSub} onChange={e => setSelSub(e.target.value)}><option value="all">Sub-Category: All</option>{filterSubCategories.map(s => <option key={s} value={s}>{s}</option>)}</select>
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-left tracking-tight">
          <thead className="bg-slate-50 text-slate-500 text-[10px] font-black border-b tracking-widest uppercase"><tr><th className="p-5 pl-8">Material Detail</th><th className="p-5">Spec</th><th className="p-5 text-center">Total Available Qty</th><th className="p-5 text-center">Actions</th></tr></thead>
            ...
            {currentItems.length > 0 ? currentItems.map((i: any) => (
              <tr key={`${i.item}-${i.spec}`} className="hover:bg-blue-50/50 transition border-b uppercase group cursor-pointer" onClick={() => setBifurcationItem(i)}>
                <td className="p-5 pl-8 leading-tight">
                  ...
                  <div className="text-[8px] text-indigo-500 mt-1 font-black tracking-widest uppercase animate-pulse opacity-0 group-hover:opacity-100">Click to see bifurcation (Split up) →</div>
                </td>
                ...
                <td className="p-5 font-bold text-center text-slate-800 text-[16px] whitespace-nowrap bg-blue-50/30">{i.totalQty} {i.unit}</td>
                <td className="p-5 text-center"><button className="bg-indigo-50 text-indigo-700 px-4 py-1.5 rounded-lg text-[10px] font-black border border-indigo-100 group-hover:bg-indigo-600 group-hover:text-white transition-all uppercase tracking-tighter shadow-sm">View Records</button></td>
              </tr>
      ...
      {/* MODAL: BIFURCATION (Details of who added what) */}
      {bifurcationItem && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-scale-in uppercase font-bold">
            <div className="p-6 border-b bg-slate-50 flex justify-between items-center">
                <div><h3 className="font-black text-slate-800 text-lg uppercase tracking-tight">Material Split-up (Bifurcation)</h3><p className="text-[10px] text-slate-400">{bifurcationItem.item} | {bifurcationItem.spec}</p></div>
                    ...
                    <tbody className="divide-y">
                        {bifurcationItem.records.map((r: any) => (
                            <tr key={r.id} className="hover:bg-slate-50">
                                ...
                                <td className="p-4 text-[10px] text-slate-500 font-medium">{new Date(r.created_at).toLocaleString('en-IN', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</td>
                                <td className="p-4 text-center font-black text-slate-800 text-[14px]">{r.qty} {r.unit}</td>
                                <td className="p-4 text-center flex justify-center gap-2">
                                    <button onClick={() => setConsumeItem(r)} className="bg-orange-600 text-white px-3 py-1.5 rounded text-[9px] font-black uppercase shadow-sm">Consume</button>
                                    <button onClick={() => { setEditItem(r); setForm({ cat: r.cat, sub: r.sub, make: r.make, model: r.model, spec: r.spec, qty: r.qty.toString(), unit: r.unit, isManual: r.is_manual }); setShowAddModal(true); }} className="bg-slate-800 text-white px-3 py-1.5 rounded text-[9px] font-black uppercase shadow-sm">Edit</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <div className="p-4 bg-slate-50 text-center"><button onClick={() => setBifurcationItem(null)} className="text-[10px] font-black text-slate-400 tracking-[0.2em] hover:text-indigo-600">Close Bifurcation</button></div>
          </div>
        </div>
      )}

      {/* MODAL: ADD / EDIT MATERIAL (Dependent Dropdowns) */}
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-scale-in uppercase font-bold">
            <div className="p-6 border-b bg-slate-50 flex justify-between items-center"><h3 className="font-black text-slate-800 text-lg uppercase tracking-tight">{editItem ? 'Edit Item Record' : 'Add New Inventory Stock'}</h3><button onClick={resetForm} className="text-slate-400 hover:text-red-500"><i className="fa-solid fa-xmark text-xl"></i></button></div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-[9px] text-slate-400 block mb-1 uppercase tracking-widest font-black">Sub-Cat</label>
                    {form.isManual ? <input type="text" disabled={!!editItem} className="w-full p-2.5 border-2 border-slate-100 rounded-xl text-xs font-bold" placeholder="e.g. Bearings" value={form.sub} onChange={e => setForm({ ...form, sub: e.target.value })} />
                      : <select disabled={!!editItem} className="w-full p-2.5 border-2 border-slate-100 rounded-xl text-xs font-bold uppercase cursor-pointer shadow-sm" value={form.sub} onChange={e => setForm({ ...form, sub: e.target.value, make: "", model: "", spec: "" })}><option value="">-- Select --</option>{availableSubs.map(s => <option key={s} value={s}>{s}</option>)}</select>}
                  </div>
                  <div><label className="text-[9px] text-slate-400 block mb-1 uppercase tracking-widest font-black">Make</label>
                    {form.isManual ? <input type="text" disabled={!!editItem} className="w-full p-2.5 border-2 border-slate-100 rounded-xl text-xs font-bold" placeholder="e.g. SKF" value={form.make} onChange={e => setForm({ ...form, make: e.target.value, model: "", spec: "" })} />
                      : <select disabled={!!editItem} className="w-full p-2.5 border-2 border-slate-100 rounded-xl text-xs font-bold uppercase cursor-pointer shadow-sm" value={form.make} onChange={e => setForm({ ...form, make: e.target.value, model: "", spec: "" })}><option value="">-- Select --</option>{availableMakes.map(m => <option key={m} value={m}>{m}</option>)}</select>}
                  </div>
                </div>
      ...
      {/* MODAL: CONSUME STOCK (Same as Image 2) */}
      {consumeItem && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[10000] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-scale-in uppercase font-bold">
            ...
            <div className="space-y-4">
              ...
              <div><label className="text-[10px] text-slate-500 font-black uppercase mb-1 tracking-widest">Purpose / Maintenance Job</label><textarea placeholder="Specify maintenance job description..." className="w-full p-4 border-2 rounded-2xl font-bold h-24 focus:border-orange-500 outline-none text-xs" value={consumeForm.note} onChange={e => setConsumeForm({ ...consumeForm, note: e.target.value })}></textarea></div>
              <button onClick={handleConsume} className="w-full py-4 bg-slate-900 text-white rounded-2xl shadow-xl font-black tracking-widest uppercase hover:bg-black">Confirm Consumption</button>
            </div>
          </div>
        </div>
      )}

      {/* Modal: Summary (Same as Image 1) */}
      {showSummary && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-scale-in uppercase font-bold">
            <div className="p-6 border-b bg-indigo-50 flex justify-between items-center"><h3 className="font-black text-indigo-900 text-lg tracking-tight uppercase"><i className="fa-solid fa-boxes-stacked mr-2"></i> Zone Stock Status</h3><button onClick={() => setShowSummary(false)} className="text-slate-400 hover:text-red-500"><i className="fa-solid fa-xmark text-xl"></i></button></div>
