...
export default function MyStoreView({ profile, fetchProfile }: any) {
  ...
  // --- 1. LOGIC: SUMMARY FUNCTION (Must be defined before use) ---
  const getSummaryData = () => {
    const summary: any = {};
    const listToSummarize = filteredList; // Summary based on current active filters
    listToSummarize.forEach(i => {
      const key = `${i.cat} > ${i.sub}`;
      if (!summary[key]) summary[key] = { cat: i.cat, sub: i.sub, total: 0 };
      summary[key].total += Number(i.totalQty || i.qty);
    });
    return Object.values(summary).sort((a: any, b: any) => a.cat.localeCompare(b.cat));
  };

  // --- 2. LOGIC: MASTER DATA DROPDOWNS ---
  ...
  const exportStoreCSV = () => {
    const headers = "Category,Sub-Cat,Item Detail,Spec,Qty,Unit,Added By,Date\n";
    const rows = filteredList.map(i => i.records.map((r:any) => `"${r.cat}","${r.sub}","${r.item}","${r.spec}","${r.qty}","${r.unit}","${r.holder_name}","${new Date(r.created_at).toLocaleDateString()}"`).join("\n"));
    ...
    <div className="animate-fade-in space-y-6 pb-20 font-roboto font-bold uppercase">
      ...
      <div className="bg-white p-6 rounded-xl border shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-2 w-full md:w-auto">
            <button onClick={exportStoreCSV} className="bg-emerald-600 text-white px-4 py-3 rounded-xl text-[10px] font-black shadow-md flex items-center gap-2 uppercase tracking-widest"><i className="fa-solid fa-file-csv"></i> Export Sheet</button>
            <button onClick={() => setShowSummary(true)} className="bg-indigo-600 text-white px-4 py-3 rounded-xl text-[10px] font-black shadow-md flex items-center gap-2 uppercase tracking-widest"><i className="fa-solid fa-chart-bar"></i> Stock Summary</button>
            <button onClick={() => { resetForm(); setShowAddModal(true); }} className="iocl-btn text-white px-5 py-3 rounded-xl text-[10px] font-black shadow-md flex items-center gap-2 uppercase tracking-widest"><i className="fa-solid fa-plus"></i> Add Stock</button>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 bg-white p-4 rounded-xl border shadow-sm">
        <div className="relative w-full">
            ...
            <input type="text" placeholder="Search Materials..." className="w-full pl-10 pr-4 py-3 border-2 border-slate-100 rounded-xl text-xs outline-none focus:border-orange-400 font-bold uppercase" value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <select className="border-2 border-slate-50 rounded-lg text-[10px] p-3 bg-white font-bold uppercase cursor-pointer" value={selCat} onChange={e => { setSelCat(e.target.value); setSelSub("all"); }}><option value="all">Category: All</option>{filterCategories.map(c => <option key={c} value={c}>{c}</option>)}</select>
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full text-left tracking-tight">
          <thead className="bg-slate-50 text-slate-500 text-[10px] font-black border-b tracking-widest uppercase"><tr><th className="p-5 pl-8">Material Detail</th><th className="p-5">Spec</th><th className="p-5 text-center">Total Balance</th><th className="p-5 text-center">Action</th></tr></thead>
      ...
      {bifurcationItem && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-scale-in uppercase font-bold">
            ...
            <div className="p-4 bg-slate-50 text-center"><button onClick={() => setBifurcationItem(null)} className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] hover:text-indigo-600">Close</button></div>
          </div>
        </div>
      )}
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[10000] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-scale-in uppercase font-bold">
            <div className="p-6 border-b bg-slate-50 flex justify-between items-center"><h3 className="font-black text-slate-800 text-lg uppercase tracking-tight">{editItem ? 'Edit Material' : 'Add New Material'}</h3><button onClick={resetForm}><i className="fa-solid fa-xmark text-xl"></i></button></div>
            <div className="p-6 space-y-4 font-bold">
              {!editItem && (
                <div className="flex justify-between items-center bg-orange-50 p-4 rounded-xl border-2 border-dashed border-orange-200 mb-2 shadow-inner">
                  <div className="flex flex-col"><span className="text-[10px] text-orange-800 font-black tracking-tight uppercase">Manual mode needed?</span><span className="text-[8.5px] text-orange-600 font-medium lowercase">toggle to type custom details</span></div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><label className="text-[9px] text-slate-400 block mb-1 uppercase tracking-widest font-black">Make</label>
                    {form.isManual ? <input type="text" disabled={!!editItem} className="w-full p-2.5 border-2 border-slate-100 rounded-xl text-xs font-bold" placeholder="e.g. L&T" value={form.make} onChange={e => setForm({ ...form, make: e.target.value, model: "", spec: "" })} />
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[10001] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-scale-in uppercase font-bold">
            ...
            <div className="space-y-4">
              <div><label className="text-[10px] text-slate-500 font-black uppercase mb-1 tracking-widest">Qty Consumed</label><input type="number" className="w-full p-4 border-2 border-slate-100 rounded-2xl font-black text-xl text-center focus:border-orange-500 outline-none" value={consumeForm.qty} onChange={e => setConsumeForm({ ...consumeForm, qty: e.target.value })} /></div>
              <div><label className="text-[10px] text-slate-500 font-black uppercase mb-1 tracking-widest">Purpose / Note</label><textarea placeholder="Specify maintenance job description..." className="w-full p-4 border-2 border-slate-100 rounded-2xl font-bold h-24 focus:border-orange-500 outline-none text-xs" value={consumeForm.note} onChange={e => setConsumeForm({ ...consumeForm, note: e.target.value })}></textarea></div>
