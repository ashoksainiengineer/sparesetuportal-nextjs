...
export default function MyStoreView({ profile, fetchProfile }: any) {
  ...
  const handleDeleteItem = async (id: number) => {
    if (confirm("Permanently delete?")) {
      ...
      resetForm(); setBifurcationItem(null); await fetchStore(); if(fetchProfile) fetchProfile();
    }
  };
  ...
  const exportCSV = () => {
    ...
    // FIXED: rows is already a string, removed rows.join
                            ...
                            <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                                <td className="p-4 pl-6 leading-tight">
                                    <div className="flex items-center gap-2 mb-1">
                                      ...
                                      {r.is_manual && <span className="bg-orange-100 text-orange-600 text-[8px] px-1.5 py-0.5 rounded font-black border border-orange-200">M</span>}
                                    </div>
            ...
            <div className="p-6 space-y-4 font-bold">
              {!editItem && (
                <div className="flex justify-between items-center bg-orange-50 p-4 rounded-xl border-2 border-dashed border-orange-200 mb-2 shadow-inner">
                  <div className="flex flex-col"><span className="text-[10px] text-orange-800 font-black uppercase tracking-tight">ITEM NOT IN LIST? (Manual Mode)</span><span className="text-[8.5px] text-orange-600 font-medium lowercase mt-1">toggle manual mode to enter details</span></div>
              ...
              <div className="space-y-3">
                <div><label className="text-[9px] text-slate-400 block mb-1 uppercase tracking-widest font-black">Category</label>
                  {form.isManual ? <input type="text" className="w-full p-2.5 border-2 border-slate-100 rounded-xl text-xs font-bold" placeholder="e.g. Electrical" value={form.cat} onChange={e => setForm({ ...form, cat: e.target.value })} />
                    : <select disabled={!!editItem} className="w-full p-2.5 border-2 border-slate-100 rounded-xl text-xs font-bold uppercase cursor-pointer" value={form.cat} onChange={e => setForm({ ...form, cat: e.target.value, sub: "", make: "", model: "", spec: "" })}><option value="">-- Choose Category --</option>{uniqueCats.map((c:any) => <option key={c} value={c}>{c}</option>)}</select>}
                </div>
