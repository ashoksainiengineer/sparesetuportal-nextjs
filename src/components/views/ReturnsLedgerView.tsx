...
export default function ReturnsLedgerView({ profile, onAction }: any) { 
    ...
    const fetchAll = async () => {
        ...
        try {
            // 1. Pending & Return Requests (Attention Required)
            ...
            // 2. Active Udhaari (Approved items only)
            ...
            // 3. Archive Logs (Returned or Rejected)
    ...
    useEffect(() => {
        if (!profile) return;
        fetchAll();
        const channel = supabase.channel('returns-audit-sync').on('postgres_changes', { event: '*', schema: 'public', table: 'requests' }, () => { 
            fetchAll(); 
    ...
    const handleProcess = async () => {
        ...
        try {
            if (type === 'approve') {
                ...
                // Mark child request as returned
                            ...
                            {pending.map(r => (
                                <tr key={r.id} className={`${r.status==='return_requested' ? 'bg-orange-50 animate-pulse' : 'bg-white'} transition border-b hover:bg-slate-50`}>
                                    ...
                                    <td className="p-4 flex gap-2 justify-center">
                                        <button onClick={()=>setActionModal({type: r.status==='pending' ? 'approve' : 'verify', data:r})} className="bg-[#ff6b00] text-white px-4 py-2 rounded-lg text-[10px] font-black shadow-md hover:bg-orange-600 tracking-widest uppercase">{r.status==='pending' ? 'Issue' : 'Verify'}</button>
                                        <button onClick={()=>setActionModal({type: 'reject', data:r})} className="bg-slate-100 text-slate-500 px-4 py-2 rounded-lg text-[9px] font-black transition tracking-widest hover:bg-slate-200 uppercase">Reject</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <section className="bg-white rounded-2xl border-t-4 border-blue-600 shadow-lg overflow-hidden flex flex-col">
                    ...
                    <div className="p-4 space-y-4 h-[500px] overflow-y-auto bg-slate-50/20">
                        {given.map(r => (
                            <div key={r.id} className="p-4 border-2 border-slate-100 bg-white rounded-2xl relative shadow-sm hover:border-blue-100 transition-colors uppercase font-bold">
                                ...
                                <div className="flex justify-between items-center bg-slate-50 p-2.5 rounded-lg mb-3">
                                    <div><p className="text-[9px] font-bold text-slate-400 uppercase">Receiver</p><p className="text-[12.5px] font-black text-slate-700 uppercase tracking-tighter">{r.from_name} ({r.from_unit})</p></div>
                                    <div className="text-right font-black text-blue-600 font-mono text-[14px]">{r.req_qty} {r.item_unit}</div>
                                </div>
                                <div className="text-[9px] font-mono text-slate-400 space-y-1 bg-slate-50/50 p-2 rounded border border-dashed tracking-tighter"><p><span className="font-black text-blue-600/70">TXN:</span> {r.txn_id || '--'}</p><p><span className="font-black text-blue-600/70">TAKEN ON:</span> {formatTS(r.timestamp)}</p></div>
                            </div>
                        ))}
                    </div>
                </section>
                ...
                <section className="bg-white rounded-2xl border-t-4 border-red-600 shadow-lg overflow-hidden flex flex-col">
                    ...
                    <div className="p-4 space-y-4 h-[500px] overflow-y-auto bg-slate-50/20">
                        {taken.map(r => (
                            <div key={r.id} className="p-4 border-2 border-slate-100 bg-white rounded-2xl relative shadow-sm hover:border-red-100 transition-colors uppercase font-bold">
                                <div className="text-slate-800 font-bold text-[14px] tracking-tight mb-1">{r.item_name}</div>
                                <div className="text-[10px] text-slate-400 mb-3 uppercase tracking-tighter">{r.item_spec}</div>
                                <div className="flex justify-between items-center bg-slate-50 p-2.5 rounded-lg mb-3">
                                    <div><p className="text-[9px] font-bold text-slate-400 uppercase">Source</p><p className="text-[12.5px] font-black text-slate-700 uppercase tracking-tighter">{r.to_unit} ({r.to_name})</p></div>
                                    <div className="text-right font-black text-red-600 font-mono text-[14px]">{r.req_qty} {r.item_unit}</div>
                                </div>
                                <div className="text-[9px] font-mono text-slate-400 mb-3 space-y-1 bg-slate-50/50 p-2 rounded border border-dashed tracking-tighter"><p><span className="font-black text-red-600/70">TXN:</span> {r.txn_id || '--'}</p><p><span className="font-black text-red-600/70">TAKEN ON:</span> {formatTS(r.timestamp)}</p></div>
            ...
            <div className="pt-10 space-y-6">
                <div className="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden">
                            ...
                            <tbody className="divide-y text-slate-600">
                                {currentArchiveLogs.map(h => (
                                    <tr key={h.id} className="hover:bg-slate-50 transition border-b">
                                        ...
                                        <td className="p-4 leading-tight"><p className="text-slate-800 font-bold text-[12.5px] tracking-tight">{h.item_name}</p><p className="text-[8.5px] text-slate-400 mt-1.5">SPEC: {h.item_spec}</p></td>
                                        <td className="p-4 text-center font-black">UDH: {h.req_qty} {h.item_unit}</td>
                    ...
                    <div className="p-4 bg-slate-50 border-t flex justify-between items-center text-[10px] font-black uppercase tracking-widest">
                        <button onClick={() => setArchivePage(prev => Math.max(prev - 1, 1))} disabled={archivePage === 1} className="px-5 py-2 bg-white border-2 rounded-lg shadow-sm disabled:opacity-30">Prev</button>
                        <span className="text-slate-400">Page {archivePage} of {Math.ceil(sortedHistory.length/logsPerPage)||1}</span>
                        <button onClick={() => setArchivePage(prev => Math.min(prev + 1, Math.ceil(sortedHistory.length/logsPerPage)||1))} disabled={archivePage === (Math.ceil(sortedHistory.length/logsPerPage)||1)} className="px-5 py-2 bg-white border-2 rounded-lg shadow-sm disabled:opacity-30">Next</button>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-scale-in">
                  ...
                  <div className="p-6 space-y-4 font-bold uppercase">
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 leading-tight">
                      <p className="text-[9px] text-slate-400 mb-1 tracking-widest">Item Name</p>
                    ...
                    <div>
                        ...
                        <textarea placeholder="Reason/Log Details..." className="w-full mt-1 p-3 border-2 rounded-lg outline-none font-bold text-xs h-24 text-slate-800 focus:border-orange-500 uppercase" onChange={e=>setForm({...form, comment:e.target.value})}></textarea>
                    </div>
