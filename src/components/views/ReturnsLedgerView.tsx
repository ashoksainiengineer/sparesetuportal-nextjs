...
export default function ReturnsLedgerView({ profile, onAction }: any) { 
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel('sparesetu-ledger-audit-v2').on('postgres_changes', { event: '*', schema: 'public', table: 'requests' }, () => { 
    ...
    const handleProcess = async () => {
        ...
        setSubmitting(true);
        try {
            const now = Date.now(); // Unified action timestamp
            ...
            if (type === 'approve') {
                ...
                const persistentTxnId = data.txn_id || `#TXN-${now.toString().slice(-6)}`;
                
                await supabase.from("requests").update({ 
                    status: 'approved', approve_comment: form.comment, txn_id: persistentTxnId, 
                    to_uid: profile.id, to_name: profile.name, req_qty: actionQty, 
                    viewed_by_requester: false, timestamp: now // FIX: Update time on action
                }).eq("id", data.id);
                ...
                await supabase.from("requests").insert([{ 
                    ...
                    txn_id: data.txn_id, timestamp: now // New request time for return
                }]);
                ...
                await supabase.from("requests").update({ status: 'returned', approve_comment: `Verified: ${form.comment}`, to_uid: profile.id, to_name: profile.name, viewed_by_requester: false, timestamp: now }).eq("id", data.id);
                const { data: inv } = await supabase.from("inventory").select("qty").eq("id", data.item_id).single();
                if (inv) await supabase.from("inventory").update({ qty: inv.qty + data.req_qty }).eq("id", data.item_id);
                alert("Verified!");
            }
            else if (type === 'reject') {
                await supabase.from("requests").update({ status: 'rejected', approve_comment: form.comment, to_uid: profile.id, to_name: profile.name, viewed_by_requester: false, timestamp: now }).eq("id", data.id);
            }
    ...
    return (
        <div className="space-y-10 animate-fade-in pb-20 font-roboto uppercase font-bold tracking-tight">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <section className="bg-white rounded-2xl border-t-4 border-blue-600 shadow-lg overflow-hidden flex flex-col h-[550px]">
                            ...
                            <tbody className="divide-y text-slate-600">
                                {currentArchiveLogs.map(h => (
                                    <tr key={h.id} className="hover:bg-slate-50 transition border-b">
                                        ...
                                        <td className="p-4 leading-tight"><p className="text-slate-800 font-bold text-[12.5px] tracking-tight">{h.item_name}</p><p className="text-[8.5px] text-slate-400 mt-1.5 uppercase">SPEC: {h.item_spec}</p></td>
                                        <td className="p-4 text-center font-black whitespace-nowrap">
                                          <div className="flex flex-col items-center gap-1 leading-none">
                                            ...
                                            {h.status === 'returned' && <span className="text-[9.5px] text-green-600 tracking-tighter">RET: {h.req_qty} {h.item_unit}</span>}
                                          </div>
                                        </td>
                                        ...
                                        {/* REFACTORED AUDIT LOG: CONDITIONAL DATA */}
                                        <td className="p-4 leading-none space-y-1.5 font-bold tracking-tighter text-[8px]">
                                            <p className="text-slate-400"><span className="opacity-50">1. REQUEST BY:</span> {h.from_name} ({h.from_unit})</p>
                                            
                                            {h.status !== 'pending' && (
                                              <p className={h.status === 'rejected' ? 'text-red-500' : 'text-blue-600'}>
                                                <span className="opacity-50">{h.status === 'rejected' ? '2. REJECTED BY:' : '2. APPROVED BY:'}</span> {h.to_name} on {formatTS(h.timestamp)}
                                              </p>
                                            )}
                                            
                                            {h.status === 'returned' && (
                                                <p className="text-green-600">
                                                  <span className="opacity-50">3. FINAL VERIFY:</span> {h.to_name} on {formatTS(h.timestamp)}
                                                </p>
                                            )}
                                        </td>
                ...
                <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden animate-scale-in">
                  ...
                  <div className="p-6 space-y-4 font-bold uppercase">
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 leading-tight">
                      <p className="text-[9px] text-slate-400 mb-1 tracking-widest uppercase">Transaction Detail</p>
                    ...
                    {(actionModal.type === 'approve' || actionModal.type === 'return') && (
                      <div>
                        <label className="text-[10px] font-black text-slate-400 tracking-widest block mb-1 uppercase">Quantity {actionModal.type === 'return' ? '(Borrowed:' : '(Requested:'} {actionModal.data.req_qty})</label>
                    ...
                    <div>
                        <label className="text-[10px] font-black text-slate-400 tracking-widest block mb-1 uppercase">Log / Comment</label>
                    ...
                    <button disabled={submitting} onClick={handleProcess} className={`w-full py-3 ${actionModal.type === 'reject' ? 'bg-red-600' : 'bg-[#ff6b00]'} text-white font-black rounded-xl shadow-lg uppercase tracking-widest text-sm hover:opacity-90 transition-opacity`}>
