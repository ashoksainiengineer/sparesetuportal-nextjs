  ...
  const handleAuth = async () => {
    ...
    try {
      if (view === "login") {
        ...
        // Forgot view ke liye check profile exists
        if (view === "forgot") {
        ...
        const res = await fetch('/api/send-otp', { 
          ...
          body: JSON.stringify({ name: form.name || 'User', email: cleanEmail, otp }) 
        });

        if (res.ok) { 
          alert("Verification OTP aapke email par bhej diya gaya hai!"); 
          setView("otp"); 
        } else { alert("OTP bhenjne mein fail!"); }
      } 
      else if (view === "otp") {
        if (form.enteredOtp === form.generatedOtp) {
          if (form.name && form.unit) { // Case: New Registration
          ...
          alert("Password Security Verified. Manual login enabled.");
          setView("login");
      }
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 login-bg font-roboto uppercase font-bold">
      <div className="w-full max-w-md login-card rounded-2xl shadow-2xl p-8 border-t-4 border-orange-500 text-center relative animate-fade-in">
        ...
        {/* BRANDING SECTION (Hamesha Top pe rahega) */}
        <div className="mb-8">
        ...
        <div className="space-y-4">
          ...
          {/* VIEW: OTP / RESET / DEFAULT EMAIL (Inputs) */}
          {view === "otp" ? (
          ...
          {/* VIEW: LOGIN / REGISTER (Password Input) */}
          ...
          {/* ACTION BUTTON (Text updates dynamically) */}
          ...
          {/* VIEW TOGGLE (Switch between Login & Register) */}
          ...
          {/* FOOTER: DEVELOPED & MAINTAINED BY (Is visible in ALL views) */}
          <div className="mt-8 pt-6 border-t border-white/10 text-center font-bold">
            ...
            {/* White names in Devnagri font for clarity */}
