        ...
        <header className="header-bg text-white p-4 sticky top-0 z-30 shadow-md flex justify-between items-center">
            <div className="flex items-center gap-6">
              ...
              <div className="flex flex-col items-center"> 
                <h1 className="font-industrial text-2xl md:text-3xl uppercase tracking-wider leading-none font-bold text-center">Gujarat Refinery</h1>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 login-bg">
      <div className="w-full max-w-md login-card rounded-2xl shadow-2xl p-8 border-t-4 border-orange-500 text-center relative">
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="mt-8 pt-4 border-t border-white/10 text-center text-slate-400">
            <p className="text-[10px] font-medium uppercase tracking-wider mb-1">Developed By</p>
            <p className="text-[11px] font-bold font-hindi">अशोक सैनी • दीपक चौहान • दिव्यांक सिंह राजपूत</p>
          </div>
        </div>
      </div>
    </div>
...
// --- GLOBAL SEARCH (Original UI Style) ---
function GlobalSearchView({ profile }: any) {
  ...
  const summaryBreakdown: any = {};
  filtered.forEach((i: any) => {
    const key = `${i.cat}||${i.sub}`;
    if (!summaryBreakdown[key]) summaryBreakdown[key] = { cat: i.cat, sub: i.sub, total: 0 };
    summaryBreakdown[key].total += i.totalQty;
  });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <section className="bg-white rounded-xl shadow-sm border overflow-hidden">
        ...
        <div className="p-4 border-b bg-slate-50/80 flex flex-wrap items-center gap-2">
             ...
             <select className="border rounded-md text-xs font-bold p-2 bg-white" onChange={(e)=>{setSelCat(e.target.value); setSelSub("all");}}>
                ...
                <option value="zero" className="text-red-500 font-bold">⚠️ Out of Stock Items</option>
             </select>
             <select className="border rounded-md text-xs font-bold p-2 bg-white" disabled={selCat === 'all' || selCat === 'zero'} value={selSub} onChange={(e)=>setSelSub(e.target.value)}>
                ...
                {[...new Set(items.filter(i => selCat === "all" ? true : i.cat === selCat).map(i => i.sub))].sort().map(s => <option key={s} value={s}>{s}</option>)}
             </select>
             ...
             <button onClick={()=>alert("Export logic attached to filtered result")} className="bg-emerald-600 text-white px-3 py-2 rounded-md text-xs font-bold shadow-sm flex items-center gap-2"><i className="fa-solid fa-file-csv"></i> Export CSV</button>
        </div>
          ...
          <tbody className="divide-y text-sm">
            {filtered.map((i: any, idx: number) => (
              <tr key={idx} className={`hover:bg-slate-50 transition border-b border-slate-50 ${i.totalQty === 0 ? 'bg-red-50/20' : ''}`}>
                <td className="p-4 pl-6 font-bold text-slate-800">{i.item}<div className="text-[10px] text-slate-400 font-bold uppercase mt-0.5">{i.cat} / {i.sub}</div></td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-scale-in max-h-[80vh] flex flex-col">
                ...
                <div className="overflow-y-auto p-4 space-y-4">
                    {Object.entries(summaryBreakdown).map(([key, row]: any) => (
                        <div key={key} className="border rounded-xl overflow-hidden shadow-sm bg-white">
                            <div className="bg-slate-100 p-2 text-[10px] font-black text-slate-600 uppercase tracking-widest border-b">{row.cat}</div>
                            <div className="p-3 pl-6 flex justify-between items-center hover:bg-slate-50 transition"><span className="text-sm font-medium text-slate-600">{row.sub}</span><span className="text-sm font-black text-indigo-600">{row.total} Nos</span></div>
                        </div>
                    ))}
                </div>
            </div>
      ...
      {/* BREAKDOWN MODAL (Bifurcation + Request) */}
      {breakdown && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
...
// --- MY LOCAL STORE (Identical Icons + Action Alert) ---
function MyStoreView({ profile, fetchProfile }: any) {
  ...
  const handleConsume = async () => {
    ...
    const { error } = await supabase.from("inventory").update({ qty: consumeItem.qty - qtyToConsume }).eq('id', consumeItem.id);
    if (!error) {
    ...
    <div className="animate-fade-in space-y-6 pb-10">
      ...
      {outOfStockCount > 0 && (
        <div className="bg-red-50 border border-red-200 p-4 rounded-xl flex items-start gap-3 animate-fade-in shadow-sm border-l-8 border-l-red-500">
          ...
          <div><h3 className="font-bold text-red-800 text-sm uppercase">Action Needed: Restock Required</h3><p className="text-xs text-red-600 font-bold">{outOfStockCount} items in your zone are currently out of stock. Please check inventory.</p></div>
        </div>
      )}

      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-xl border shadow-sm gap-4">
        <div><h2 className="text-xl font-bold text-slate-800 font-industrial uppercase">My Local Store</h2><p className="text-xs font-bold bg-blue-50 text-blue-700 px-2 py-0.5 rounded mt-1 uppercase tracking-tighter inline-block">Managing: {profile?.unit}</p></div>
        <div className="flex gap-2">
          <button onClick={()=>alert("Exporting Store CSV...")} className="bg-emerald-600 text-white px-4 py-2.5 rounded-xl font-bold text-xs shadow-md transition flex items-center gap-2 hover:bg-emerald-700"><i className="fa-solid fa-file-csv"></i> Export CSV</button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        {/* FILTERS INSIDE MY STORE */}
        <div className="p-4 border-b bg-slate-50/80 flex flex-wrap items-center gap-2">
             <div className="relative flex-grow md:w-64"><i className="fa-solid fa-search absolute left-3 top-3 text-slate-400"></i><input type="text" placeholder="Filter My Store..." className="w-full pl-9 pr-4 py-2 border rounded-md text-sm outline-none shadow-inner" onChange={(e)=>setSearch(e.target.value)} /></div>
             <select className="border rounded-md text-[11px] font-bold p-2 bg-white" onChange={(e)=>setSelCat(e.target.value)}>
                <option value="all">Category: All</option>
                {[...new Set(myItems.map(i => i.cat))].sort().map(c => <option key={c} value={c}>{c}</option>)}
             </select>
        </div>
        <div className="overflow-x-auto"><table className="w-full text-left">
          <thead className="bg-slate-50 text-slate-500 text-[10px] font-bold border-b uppercase"><tr><th className="p-5">Category</th><th className="p-5">Item Name</th><th className="p-5">Spec</th><th className="p-5 text-center">Qty</th><th className="p-5 text-center">Manage</th></tr></thead>
          <tbody className="divide-y text-sm">
            {filtered.map(i => (
              <tr key={i.id} className={`${Number(i.qty) === 0 ? 'bg-red-50/30' : 'hover:bg-slate-50'} transition border-b border-slate-50`}>
                ...
                <td className="p-5 font-bold text-center">{Number(i.qty) === 0 ? <span className="text-red-600 font-black text-[10px] uppercase bg-red-100 px-2 py-1 rounded border border-red-200">Out of Stock</span> : <span className="text-emerald-600 text-lg font-black">{i.qty} <small className="text-[10px] text-slate-400">Nos</small></span>}</td>
                <td className="p-5 flex gap-3 justify-center items-center">
                    {/* ORIGINAL BOX AND PEN ICONS */}
                    <button onClick={()=>setConsumeItem(i)} disabled={Number(i.qty) === 0} className="text-indigo-600 hover:scale-125 transition disabled:opacity-30" title="Consume Stock"><i className="fa-solid fa-box-open text-xl"></i></button>
                    <button onClick={()=>setEditItem(i)} className="text-slate-400 hover:text-blue-500 hover:scale-125 transition" title="Edit Qty"><i className="fa-solid fa-pen-to-square text-xl"></i></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {consumeItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-scale-in">
            <div className="bg-indigo-600 px-6 py-4 border-b flex justify-between items-center text-white"><h3 className="text-lg font-bold">Consume Stock</h3><button onClick={()=>setConsumeItem(null)} className="text-white hover:text-red-200">✕</button></div>
            <div className="p-6 space-y-4 text-center">
                <div className="text-sm font-bold text-slate-700 uppercase tracking-tighter">{consumeItem.item}</div>
                <div className="bg-slate-50 p-4 rounded-xl border border-dashed border-slate-300"><p className="text-[10px] font-black text-slate-400 uppercase">Available Units</p><p className="text-3xl font-black text-emerald-600">{consumeItem.qty} Nos</p></div>
                <div><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest pl-1 mb-1 block">Quantity to Remove</label><input type="number" id="cQty" placeholder="1" className="w-full p-4 border-2 border-slate-200 rounded-xl text-2xl font-black text-center focus:border-indigo-500 outline-none" /></div>
                <button onClick={handleConsume} className="w-full py-4 bg-indigo-600 text-white font-black rounded-xl shadow-lg uppercase tracking-widest hover:bg-indigo-700 transition">Confirm Job Done</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-scale-in p-6">
            <div className="flex justify-between items-center mb-6 border-b pb-2"><h3 className="text-lg font-bold uppercase text-blue-800">Correct Quantity</h3><button onClick={()=>setEditItem(null)} className="text-slate-300">✕</button></div>
            <p className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-tighter">{editItem.item}</p>
            <input type="number" id="eQty" defaultValue={editItem.qty} className="w-full p-4 border-2 rounded-xl text-center text-3xl font-black mb-4 focus:border-blue-500 outline-none shadow-inner" />
            <button onClick={async ()=>{
                const nQty = parseInt((document.getElementById('eQty') as HTMLInputElement).value);
                const {error} = await supabase.from('inventory').update({qty: nQty}).eq('id', editItem.id);
                if(!error){ alert("Corrected!"); fetch(); setEditItem(null); }
            }} className="w-full py-4 bg-blue-600 text-white font-black rounded-xl uppercase tracking-widest hover:bg-blue-700 shadow-md">Apply Correction</button>
          </div>
        </div>
      )}

      {/* ADD STOCK MODAL (Identical Look) */}
      {showAddModal && (
            ...
            <div className="space-y-4">
              {!form.isManual ? (<>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Sub Category</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-white disabled:bg-slate-50" disabled={!form.cat} value={form.sub} onChange={e=>setForm({...form, sub: e.target.value, make:"", model:"", spec:""})}><option value="">-- Select --</option>{subs.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Make</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-white disabled:bg-slate-50" disabled={!form.sub} value={form.make} onChange={e=>setForm({...form, make: e.target.value, model:"", spec:""})}><option value="">-- Select --</option>{makes.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                </div>
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Model</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-white disabled:bg-slate-50" disabled={!form.make} value={form.model} onChange={e=>setForm({...form, model: e.target.value, spec:""})}><option value="">-- Select --</option>{models.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Specification</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-white disabled:bg-slate-50" disabled={!form.model} value={form.spec} onChange={e=>setForm({...form, spec: e.target.value})}><option value="">-- Select --</option>{specs.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
              </>) : (
                <div className="space-y-4">
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Material Name</label><input type="text" placeholder="e.g. Copper Transformer" className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm outline-none focus:border-orange-500" onChange={e=>setForm({...form, model: e.target.value})} /></div>
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Technical Details</label><input type="text" placeholder="Spec details..." className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm outline-none focus:border-orange-500" onChange={e=>setForm({...form, spec: e.target.value})} /></div>
                </div>
              )}
              <div className="grid grid-cols-2 gap-3 border-t pt-4">
                  <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1 text-center block">Quantity</label><input type="number" placeholder="0" className="w-full p-3 border-2 border-slate-100 rounded-lg text-2xl font-black text-center outline-none focus:border-orange-500" onChange={e=>setForm({...form, qty: e.target.value})} /></div>
                  <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1 text-center block">Unit</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-slate-50 h-full"><option>Nos</option><option>Mtrs</option><option>Set</option></select></div>
              </div>
...
function UsageHistoryView({ profile }: any) {
  ...
  return (
    <section className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-fade-in">
        <div className="p-5 border-b bg-slate-50/50 flex justify-between items-center"><h2 className="text-lg font-bold text-slate-800 font-industrial uppercase">Consumption Logs</h2><span className="text-[10px] font-black bg-slate-200 px-2 py-1 rounded uppercase tracking-widest">Zone Logs</span></div>
        <div className="overflow-x-auto"><table className="w-full text-left">
          <thead className="bg-slate-50 text-slate-500 text-[10px] font-bold border-b uppercase"><tr><th className="p-5">Date & Time</th><th className="p-5">Details</th><th className="p-5 text-center">Qty</th></tr></thead>
          <tbody className="divide-y text-sm">
            {logs.map(l => (
              <tr key={l.id} className="hover:bg-slate-50 transition border-b border-slate-50">
                ...
                <td className="p-5 font-bold text-slate-800">{l.item_name}<div className="text-[10px] text-slate-400 uppercase mt-0.5">{l.category}</div></td>
                <td className="p-5 text-center font-black text-red-600">-{l.qty_consumed} <small className="text-[10px] uppercase">Nos</small></td>
              </tr>
            ))}
          </tbody>
        </table></div>
        {logs.length === 0 && <div className="p-20 text-center italic text-slate-400">No usage logs found.</div>}
    </section>
  );
}
...
function MonthlyAnalysisView({ profile }: any) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="bg-white p-6 rounded-xl border flex justify-between items-center"><h2 className="text-xl font-bold font-industrial uppercase tracking-widest text-slate-700">Usage Analysis</h2></div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {analysis.map((a, idx) => (
          <div key={idx} className="bg-white p-6 rounded-2xl border shadow-sm text-center transition hover:shadow-md">
            ...
            <div className="text-[10px] font-bold text-emerald-500 mt-2 uppercase tracking-tight">{a.count} Consumptions</div>
          </div>
        ))}
      </div>
      {analysis.length === 0 && <div className="p-20 text-center italic text-slate-400 bg-white rounded-xl border">Insufficient data for chart analysis.</div>}
    </div>
  );
}
    ...
    return (
        <section className="animate-fade-in space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm min-h-[400px] flex flex-col"><h3 className="font-bold text-slate-400 text-[10px] uppercase mb-6 flex items-center gap-2 tracking-widest border-b pb-2"><i className="fa-solid fa-arrow-up text-blue-500"></i> Udhaari Diya (Lent)</h3><p className="text-center text-slate-300 mt-32 italic text-sm">No items out on loan.</p></div>
                <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm min-h-[400px] flex flex-col"><h3 className="font-bold text-slate-400 text-[10px] uppercase mb-6 flex items-center gap-2 tracking-widest border-b pb-2"><i className="fa-solid fa-arrow-down text-red-500"></i> Udhaari Liya (Borrowed)</h3><p className="text-center text-slate-300 mt-32 italic text-sm">No items borrowed from others.</p></div>
            </div>
        </section>
    );
}