  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f1f5f9]">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto relative pb-20 md:pb-0">
        {/* HEADER (Original Professional Look with White Logo Text) */}
        ...
        <div className="p-4 md:p-10 max-w-7xl mx-auto w-full space-y-8">
          ...
          {activeTab === "returns" && <div className="bg-white p-20 rounded-xl border text-center italic text-slate-400 border-dashed">Udhaari logic linked.</div>}
        </div>
      </main>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 login-bg">
      <div className="w-full max-w-md login-card rounded-2xl shadow-2xl p-8 border-t-4 border-orange-500 text-center relative animate-fade-in">
        ...
        <div className="space-y-4">
          ...
          {view === "otp" ? <input type="text" placeholder="######" maxLength={6} className="w-full p-3 rounded-lg login-input text-center text-2xl tracking-[0.5em] font-bold text-white outline-none" onChange={e=>setForm({...form, enteredOtp:e.target.value})} /> : <input type="email" value={form.email} className="w-full p-3 rounded-lg login-input text-sm" placeholder="Official Email ID" onChange={e=>setForm({...form, email:e.target.value})} />}
          ...
          <button onClick={handleAuth} className="w-full h-12 mt-4 iocl-btn text-white font-bold rounded-lg shadow-lg flex items-center justify-center gap-2 uppercase tracking-widest">{authLoading ? "..." : view === 'login' ? "Secure Login" : "Continue"}</button>
...
function GlobalSearchView({ profile }: any) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {showSummary && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-scale-in max-h-[80vh] flex flex-col">
                <div className="bg-indigo-50 px-6 py-4 border-b flex justify-between items-center"><h3 className="text-lg font-bold text-indigo-900 uppercase tracking-widest">Stock Summary Breakdown</h3><button onClick={()=>setShowSummary(false)} className="text-slate-400 font-bold">✕</button></div>
                <div className="overflow-y-auto p-4 space-y-2">
                    {Object.entries(summaryMap).map(([cat, qty]) => (
                        <div key={cat} className="flex justify-between p-3 border-b text-sm font-bold hover:bg-slate-50 transition uppercase"><span className="text-slate-500">{cat}</span><span className="text-indigo-600">{qty} Nos</span></div>
                    ))}
                </div>
            </div>
        </div>
      ...
      {/* BREAKDOWN MODAL logic same as before... */}
      {breakdown && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
...
// --- MY LOCAL STORE (RESTOCK ALERT + FULL EDIT + STYLISH POPUP) ---
function MyStoreView({ profile, fetchProfile }: any) {
  ...
  const [selCat, setSelCat] = useState("all"); // FIXED: Error "setSelCat" not found
  ...
  const handleSave = async () => {
    ...
    if (!error) { fetch(); setShowAddModal(false); }
  };
  ...
  const handleConsumeAction = async () => {
    const qtyToConsume = parseInt(form.qty);
    ...
    if (!error) {
        await supabase.from("usage_logs").insert([{ item_name: consumeItem.item, category: consumeItem.cat, spec: consumeItem.spec, qty_consumed: qtyToConsume, note: form.note, consumer_uid: profile.id, consumer_name: profile.name, consumer_unit: profile.unit, timestamp: Date.now() }]);
    ...
    <div className="animate-fade-in space-y-6 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-xl border shadow-sm gap-4">
        ...
        <div className="flex gap-2">
            <button onClick={() => { const csv = "Category,Item Name,Spec,Qty\n" + myItems.map(i => `"${i.cat}","${i.item}","${i.spec}","${i.qty}"`).join("\n"); const blob = new Blob([csv], { type: 'text/csv' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'My_Local_Store.csv'; a.click(); }} className="bg-emerald-600 text-white px-4 py-2.5 rounded-xl font-bold text-xs shadow-md transition flex items-center gap-2 hover:bg-emerald-700"><i className="fa-solid fa-file-csv"></i> Export CSV</button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-slate-50 text-slate-500 text-[10px] font-bold border-b uppercase"><tr><th className="p-5 pl-8">Category</th><th className="p-5">Item Name</th><th className="p-5">Spec</th><th className="p-5 text-center">Qty</th><th className="p-5 text-center">Manage</th></tr></thead>
          <tbody className="divide-y text-sm">{filtered.map(i => (<tr key={i.id} className={`${Number(i.qty) === 0 ? 'bg-red-50/30' : 'hover:bg-slate-50'} transition border-b border-slate-50`}><td className="p-5 pl-8 text-[10px] font-bold text-slate-400 uppercase">{i.cat}</td><td className="p-5 font-bold text-slate-800 leading-tight">{i.item}</td><td className="p-5"><span className="bg-white border px-2 py-1 rounded text-[11px] font-medium text-slate-600 shadow-sm">{i.spec}</span></td><td className="p-5 font-bold text-center"><span className={`${Number(i.qty) === 0 ? 'text-red-600' : 'text-emerald-600'} text-lg font-black`}>{i.qty} Nos</span></td><td className="p-5 flex gap-3 justify-center items-center"><button onClick={()=>setConsumeItem(i)} className="text-indigo-600 hover:scale-125 transition"><i className="fa-solid fa-box-open text-xl"></i></button><button onClick={()=>{ setEditItem(i); setForm({cat:i.cat, sub:i.sub, make:i.make, model:i.model, spec:i.spec, qty:i.qty.toString(), note:"", isManual: i.cat === "Manual Entry"}); }} className="text-slate-400 hover:text-blue-500 hover:scale-125 transition"><i className="fa-solid fa-pen-to-square text-xl"></i></button></td></tr>))}</tbody></table></div>
      </div>
      ...
      {consumeItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden animate-scale-in text-center p-6">
            ...
            <div><label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-1">Consumption Note / Job Name</label><textarea placeholder="Job details..." className="w-full p-3 border-2 border-slate-100 rounded-xl text-sm h-20 outline-none focus:border-indigo-500 resize-none" onChange={e=>setForm({...form, note: e.target.value})} /></div>
            <div className="flex gap-2 pt-2"><button onClick={()=>setConsumeItem(null)} className="flex-1 py-4 bg-slate-100 text-slate-500 font-bold rounded-2xl uppercase tracking-widest text-xs hover:bg-slate-200">Back</button><button onClick={handleConsumeAction} className="flex-2 py-4 bg-indigo-600 text-white font-black rounded-2xl shadow-lg uppercase tracking-widest text-xs hover:bg-indigo-700">Confirm</button></div>
          </div>
        </div>
      )}

      {/* EDIT MODAL and ADD STOCK MODAL logic remains same... */}
      {editItem && (
...
function UsageHistoryView({ profile }: any) {
  ...
  return (<section className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm"><div className="p-5 border-b bg-slate-50/50 flex justify-between items-center"><h2 className="text-lg font-bold text-slate-800 font-industrial uppercase">Consumption Logs</h2><span className="text-[10px] font-black bg-indigo-50 text-indigo-600 px-2 py-1 rounded uppercase tracking-widest">Job History</span></div><div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-slate-50 text-slate-500 text-[10px] font-bold border-b uppercase"><tr><th className="p-5 pl-8">Date & Time</th><th className="p-5">Material Details</th><th className="p-5 text-center">Qty</th><th className="p-5">Job Note</th></tr></thead><tbody className="divide-y text-sm">{logs.map(l => (<tr key={l.id} className="hover:bg-slate-50 transition border-b border-slate-50"><td className="p-5 pl-8 text-xs text-slate-500 font-bold leading-tight">{new Date(Number(l.timestamp)).toLocaleDateString()}</td><td className="p-5 font-bold text-slate-800 leading-tight">{l.item_name}<div className="text-[10px] text-slate-400 uppercase mt-0.5">{l.category}</div></td><td className="p-5 text-center font-black text-red-600">-{l.qty_consumed} Nos</td><td className="p-5 text-xs text-slate-500 italic truncate max-w-[200px]">{l.note || "-"}</td></tr>))}</tbody></table></div>{logs.length===0 && <div className="p-20 text-center italic text-slate-400">No records found.</div>}</section>);
}

function MonthlyAnalysisView({ profile }: any) {
  const [analysis, setAnalysis] = useState<any[]>([]);
  useEffect(() => { const f = async () => { const { data } = await supabase.from("usage_logs").select("*").eq("consumer_uid", profile.id); if (data) { const stats: any = {}; data.forEach((l: any) => { const month = new Date(Number(l.timestamp)).toLocaleString('default', { month: 'long', year: 'numeric' }); if (!stats[month]) stats[month] = { month, total: 0, count: 0 }; stats[month].total += Number(l.qty_consumed); stats[month].count += 1; }); setAnalysis(Object.entries(stats).map(([m, d]: any) => ({ month: m, ...d }))); } }; if (profile) f(); }, [profile]);
