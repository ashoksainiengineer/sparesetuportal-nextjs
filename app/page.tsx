          ...
          <tbody className="divide-y text-sm">
            {filtered.map((i: any, idx: number) => (
              <tr key={idx} className={`hover:bg-slate-50 transition border-b border-slate-50 ${i.qty === 0 ? 'bg-red-50/20' : ''}`}>
                ...
                <td className="p-4"><span className="bg-white border px-2 py-1 rounded text-[11px] font-bold text-slate-600 shadow-sm uppercase font-inter">{i.spec}</span></td>
                <td className="p-4 text-center font-mono font-bold whitespace-nowrap">{i.qty} {i.unit}</td>
    ...
    <div className="animate-fade-in space-y-6 pb-10 uppercase font-inter font-bold">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden font-mono uppercase font-bold">
        <div className="overflow-x-auto"><table className="w-full text-left font-industrial tracking-tight font-inter"><thead className="bg-slate-50 text-slate-500 text-[10px] font-bold border-b uppercase font-industrial tracking-widest"><tr><th className="p-5 pl-8">Category</th><th className="p-5">Item Name</th><th className="p-5">Spec</th><th className="p-5 text-center">Qty</th></tr></thead>
          <tbody className="divide-y text-sm">
              {myItems.map(i => (<tr key={i.id} className="hover:bg-slate-50 transition border-b border-slate-50 font-bold"><td className="p-5 pl-8 text-[10px] font-bold text-slate-400 uppercase leading-none">{i.cat}</td><td className="p-5 font-bold text-slate-800 leading-tight uppercase font-inter">{i.item}</td><td className="p-5 font-mono uppercase font-bold"><span className="bg-white border px-2 py-1 rounded text-[11px] font-bold text-slate-600 shadow-sm uppercase">{i.spec}</span></td><td className="p-5 font-bold text-center font-mono uppercase whitespace-nowrap">{i.qty} {i.unit}</td></tr>))}
          </tbody>
        </table></div>
      </div>
