    ...
    <div className="animate-fade-in space-y-6 pb-10 uppercase font-inter font-bold">
      ...
      {showAddModal && <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto font-industrial"><div className="bg-white w-full max-w-[360px] rounded-2xl shadow-2xl p-5 relative animate-scale-in my-auto border-t-4 border-slate-900 font-inter font-bold uppercase"><button onClick={() => setShowAddModal(false)} className="absolute top-4 right-4 text-slate-400 font-bold text-xl">✕</button><h3 className="text-sm font-black text-slate-800 mb-6 border-b pb-2 uppercase tracking-widest text-center font-industrial uppercase font-bold">Inward Catalog Entry</h3><div className="space-y-3 font-industrial font-bold uppercase">
          ...
          <div className="grid grid-cols-2 gap-3 uppercase font-bold font-industrial">
              <div className="flex flex-col"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest font-bold">Sub Cat</label><select className="w-full p-2 border-2 border-slate-100 rounded-lg text-xs outline-none uppercase font-bold h-[38px] font-industrial" value={form.sub} onChange={e=>setForm({...form, sub: e.target.value})}><option value="">Sub</option>{[...new Set(masterCatalog.filter(i=>i.cat===form.cat).map(i=>i.sub))].map(s=><option key={s} value={s}>{s}</option>)}</select></div>
              <div className="flex flex-col"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest font-bold">Make</label><select className="w-full p-2 border-2 border-slate-100 rounded-lg text-xs outline-none uppercase font-bold h-[38px] font-industrial" value={form.make} onChange={e=>setForm({...form, make: e.target.value})}><option value="">Make</option>{[...new Set(masterCatalog.filter(i=>i.cat===form.cat && i.sub===form.sub).map(i=>i.make))].map(m=><option key={m} value={m}>{m}</option>)}</select></div>
          </div>
          <div><label className="text-[9px] font-black text-slate-400 uppercase font-bold font-industrial">Model Name</label><select className="w-full p-2 border-2 border-slate-100 rounded-lg text-xs outline-none uppercase font-bold font-industrial" value={form.model} onChange={e=>setForm({...form, model: e.target.value})}><option value="">Select Model</option>{[...new Set(masterCatalog.filter(i=>i.cat===form.cat && i.sub===form.sub && i.make===form.make).map(i=>i.model))].map(m=><option key={m} value={m}>{m}</option>)}</select></div>
          <div><label className="text-[9px] font-black text-slate-400 uppercase font-bold font-industrial">Spec Details</label><select className="w-full p-2 border-2 border-slate-100 rounded-lg text-xs outline-none uppercase font-bold font-industrial" value={form.spec} onChange={e=>setForm({...form, spec: e.target.value})}><option value="">Select Spec</option>{[...new Set(masterCatalog.filter(i=>i.cat===form.cat && i.sub===form.sub && i.make===form.make && i.model===form.model).map(i=>i.spec))].map(s=><option key={s} value={s}>{s}</option>)}</select></div>
                    ...
                    <tbody className="divide-y text-slate-600 uppercase font-bold">
                        {pending.map(r => (
                            <tr key={r.id} className={`${r.status==='return_requested' ? 'bg-orange-50 animate-pulse' : 'bg-white'} transition border-b`}>
                                <td className="p-4 pl-6 leading-tight font-inter">
                                  <div className="text-slate-800 font-bold text-[13.5px] tracking-tight uppercase">{r.item_name}</div>
                                  <div className="text-[9.5px] text-slate-400 font-bold mt-1 uppercase tracking-tighter font-mono">{r.item_spec}</div>
                                  <div className="text-[8.5px] text-slate-300 italic mt-1 font-mono uppercase">{formatTS(r.timestamp)}</div>
                                </td>
                                ...
                                <td className="p-4 text-center font-black text-orange-600 text-[14px] font-mono whitespace-nowrap uppercase">{r.req_qty} {r.item_unit}</td>
                    ...
                    <div className="p-4 space-y-4 max-h-[500px] overflow-y-auto">
                        {given.map(r => (
                            <div key={r.id} className="p-4 border-2 border-slate-100 bg-white rounded-2xl relative shadow-sm uppercase font-bold font-inter">
                                <div className="text-slate-800 font-bold text-[13.5px] tracking-tight mb-1 uppercase">{r.item_name}</div>
                                <div className="text-[9.5px] text-slate-400 mb-3 uppercase tracking-tighter font-bold uppercase">{r.item_spec}</div>
                                <div className="flex justify-between items-center bg-slate-50 p-2.5 rounded-lg mb-3"><div><p className="text-[9px] font-bold text-slate-400 uppercase font-industrial uppercase">Receiver</p><p className="text-[12.5px] font-black text-slate-700 uppercase tracking-tighter uppercase">{r.from_name} ({r.from_unit})</p></div><div className="text-right font-black text-blue-600 font-mono text-[14px] uppercase">{r.req_qty} {r.item_unit}</div></div>
                    ...
                    <div className="p-4 space-y-4 max-h-[500px] overflow-y-auto">
                        {taken.map(r => (
                            <div key={r.id} className="p-4 border-2 border-slate-100 bg-white rounded-2xl relative uppercase font-bold font-inter">
                                <div className="text-slate-800 font-bold text-[13.5px] tracking-tight mb-1 uppercase">{r.item_name}</div>
                                <div className="text-[9.5px] text-slate-400 mb-3 uppercase tracking-tighter font-bold uppercase">{r.item_spec}</div>
                                <div className="flex justify-between items-center bg-slate-50 p-2.5 rounded-lg mb-3 font-bold uppercase"><div><p className="text-[9px] font-bold text-slate-400 uppercase font-industrial uppercase">Source</p><p className="text-[12.5px] font-black text-slate-700 uppercase tracking-tighter uppercase">{r.to_unit} ({r.to_name})</p></div><div className="text-right font-black text-red-600 font-mono text-[14px] uppercase">{r.req_qty} {r.item_unit}</div></div>
                            ...
                            {[...givenHistory, ...takenHistory].sort((a,b)=>Number(b.timestamp)-Number(a.timestamp)).map(h => (
                                <tr key={h.id} className="hover:bg-slate-50 transition border-b uppercase font-bold uppercase">
                                    <td className="p-4 leading-tight uppercase font-bold uppercase">
                                      <p className="text-slate-800 font-bold text-[12.5px] tracking-tight uppercase">{h.item_name}</p>
                                      <p className="text-[8.5px] text-slate-400 mt-1.5 uppercase uppercase">SPEC: {h.item_spec}</p>
                                    </td>
                                    ...
                                    <td className="p-4 text-center uppercase font-bold uppercase"><span className={`px-2.5 py-1 rounded-full text-[8.5px] font-black uppercase tracking-widest uppercase ${h.status==='returned' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{h.status}</span></td>
                                </tr>
                            ))}
                        </tbody></table></div>
                </div>
