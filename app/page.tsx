  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f1f5f9]">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto relative pb-20 md:pb-0">
        {/* --- HEADER WITH ACCURATE LOGO & TAGLINES --- */}
...
// --- AUTH VIEW (100% IDENTICAL DESIGN) ---
function AuthView() {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 login-bg">
      <div className="w-full max-w-md login-card rounded-2xl shadow-2xl p-8 fade-in border-t-4 border-orange-500 text-center relative">
        ...
        <div className="space-y-4">
          ...
          {view === "otp" ? (
             <div className="relative"><i className="fa-solid fa-key absolute left-4 top-3.5 text-slate-400"></i><input type="text" placeholder="######" value={form.enteredOtp} className="w-full pl-10 pr-4 py-3 rounded-lg login-input text-center text-2xl tracking-[0.5em] font-bold text-white outline-none" onChange={e=>setForm({...form, enteredOtp:e.target.value})} /></div>
          ) : (
             <div className="relative"><i className="fa-solid fa-envelope absolute left-4 top-3.5 text-slate-400"></i><input type="email" placeholder="Official Email ID" value={form.email} className="w-full pl-10 pr-4 py-3 rounded-lg login-input outline-none text-sm" onChange={e=>setForm({...form, email:e.target.value})} /></div>
          )}
          {view !== "otp" && <div className="relative"><i className="fa-solid fa-lock absolute left-4 top-3.5 text-slate-400"></i><input type="password" placeholder="Password" className="w-full pl-10 pr-4 py-3 rounded-lg login-input outline-none text-sm" onChange={e=>setForm({...form, pass:e.target.value})} /></div>}
          
          {view === "login" && <div className="text-right"><button onClick={()=>alert("Reset link email check karein")} className="text-xs text-orange-500 hover:text-orange-400 font-bold transition">Forgot Password?</button></div>}

          <button onClick={handleAuth} className="w-full h-12 mt-4 iocl-btn text-white font-bold rounded-lg shadow-lg flex items-center justify-center gap-2 uppercase tracking-widest">{authLoading ? "Processing..." : view === 'login' ? "Secure Login" : "Continue"}</button>
          
          <div className="mt-6 text-center border-t border-white/10 pt-4">
            <p className="text-xs text-slate-400">{view==='login' ? "New User? " : "Already have an account? "}
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <section className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="p-4 border-b bg-slate-50/80 flex flex-wrap items-center gap-2">
             ...
             <button onClick={()=>alert("Exporting Global CSV...")} className="bg-emerald-600 text-white px-3 py-2 rounded-md text-xs font-bold flex items-center gap-2 shadow-sm"><i className="fa-solid fa-file-csv"></i> Export CSV</button>
        </div>
      ...
      {showSummary && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-scale-in max-h-[80vh] flex flex-col">
                <div className="bg-indigo-50 px-6 py-4 border-b flex justify-between items-center"><h3 className="text-lg font-bold text-indigo-900 uppercase tracking-widest">Total Stock Summary</h3><button onClick={()=>setShowSummary(false)}>✕</button></div>
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-10">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b bg-slate-50/80 flex flex-wrap items-center gap-2">
             <div className="relative flex-grow md:w-64"><i className="fa-solid fa-search absolute left-3 top-3 text-slate-400"></i><input type="text" placeholder="Search My Store..." className="w-full pl-9 pr-4 py-2 border rounded-md text-sm outline-none shadow-inner" onChange={(e)=>setSearch(e.target.value)} /></div>
      ...
      {consumeItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-scale-in text-center p-6">
            <h3 className="text-lg font-bold text-slate-800 uppercase mb-4 border-b pb-2">Consume Stock</h3>
            <div className="text-sm font-bold text-slate-700 mb-4">{consumeItem.item}</div>
            <div className="bg-slate-50 p-4 rounded-xl border border-dashed mb-4"><p className="text-[10px] font-black text-slate-400 uppercase">Available</p><p className="text-3xl font-black text-emerald-600">{consumeItem.qty} Nos</p></div>
            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1 block">Quantity Used</label>
            <input type="number" id="cQty" placeholder="1" className="w-full p-4 border-2 border-slate-200 rounded-xl text-2xl font-black text-center focus:border-indigo-500 outline-none mb-4" />
            <button onClick={async ()=>{
                const qUsed = parseInt((document.getElementById('cQty') as HTMLInputElement).value);
                ...
                if(!error){
                    ...
                    alert("Recorded!"); fetch(); setConsumeItem(null);
                }
            ...
            <button onClick={()=>setConsumeItem(null)} className="w-full mt-2 text-xs font-bold text-slate-400 uppercase underline">Cancel</button>
          </div>
        </div>
      )}
      ...
      {editItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-scale-in p-6 text-center">
            <div className="flex justify-between items-center mb-6 border-b pb-2 uppercase"><h3 className="text-lg font-bold text-blue-800">Correct Qty</h3><button onClick={()=>setEditItem(null)}>✕</button></div>
            <p className="text-[10px] font-black text-slate-400 uppercase mb-4">{editItem.item}</p>
            <input type="number" id="eQty" defaultValue={editItem.qty} className="w-full p-4 border-2 rounded-xl text-center text-4xl font-black mb-6 focus:border-blue-500 outline-none shadow-inner" />
            <button onClick={async ()=>{
                ...
                if(!error){ alert("Updated!"); fetch(); setEditItem(null); }
            }} className="w-full py-4 bg-blue-600 text-white font-black rounded-xl uppercase tracking-widest hover:bg-blue-700 shadow-md">Apply Update</button>
          </div>
        </div>
      )}
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-8 relative animate-scale-in my-auto">
            ...
            <div className="space-y-4">
              {!form.isManual ? (<>
                ...
                <div className="space-y-4">
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Item Name</label><input type="text" placeholder="Material Name" className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm outline-none focus:border-orange-500" onChange={e=>setForm({...form, model: e.target.value})} /></div>
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Specification</label><input type="text" placeholder="Technical Details" className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm outline-none focus:border-orange-500" onChange={e=>setForm({...form, spec: e.target.value})} /></div>
                </div>
              )}
