    ...
    <div className="flex h-screen overflow-hidden bg-[#f1f5f9] font-roboto">
      <aside className="w-64 bg-white hidden md:flex flex-col flex-shrink-0 z-20 shadow-xl border-r border-slate-200">
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50">
          ...
          <button onClick={() => supabase.auth.signOut().then(()=>window.location.reload())} className="w-full py-2 text-xs text-red-500 hover:bg-red-50 font-bold rounded-lg transition border border-red-100 text-center block uppercase font-roboto">Logout</button>
        </div>
      </aside>
  ...
  return (
    <div className="animate-fade-in space-y-6 pb-10 uppercase font-roboto font-bold">
      ...
      {showAddModal && <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md overflow-y-auto"><div className="bg-white w-full max-w-[360px] rounded-2xl shadow-2xl p-5 relative animate-scale-in my-auto border-t-4 border-slate-900 font-roboto font-bold uppercase"><button onClick={() => setShowAddModal(false)} className="absolute top-4 right-4 text-slate-400 font-bold text-xl font-roboto font-bold">✕</button><h3 className="text-sm font-black text-slate-800 mb-6 border-b pb-2 uppercase tracking-widest text-center font-bold font-roboto font-bold">Inward Catalog Entry</h3><div className="space-y-3 font-bold uppercase font-roboto font-bold">
          ...
          <button onClick={handleSave} className="w-full py-2.5 bg-slate-900 text-white font-black rounded-lg shadow-lg mt-2 uppercase text-[9px] tracking-widest hover:bg-slate-800 font-bold uppercase font-roboto">Complete Entry</button>
      </div></div></div>}
    </div>
  );
}
            ...
            <section className="bg-white rounded-xl border-t-4 border-orange-500 shadow-xl overflow-hidden font-roboto">
                ...
                <div className="overflow-x-auto font-roboto font-bold"><table className="w-full text-left text-sm divide-y font-mono font-bold uppercase font-roboto">
                    ...
                    <tbody className="divide-y text-slate-600 uppercase font-bold font-roboto">
                        {pending.map(r => (
                            <tr key={r.id} className={`${r.status==='return_requested' ? 'bg-orange-50 animate-pulse' : 'bg-white'} transition border-b font-roboto uppercase`}>
                                <td className="p-4 pl-6 leading-tight uppercase font-roboto">
                                  <div className="text-slate-800 font-bold text-[14px] tracking-tight uppercase font-roboto">{r.item_name}</div>
                                  <div className="text-[10px] text-slate-400 font-bold mt-1 uppercase tracking-tighter font-mono font-roboto">{r.item_spec}</div>
                                  <div className="text-[8.5px] text-slate-300 italic mt-1 font-mono font-roboto">{formatTS(r.timestamp)}</div>
                                </td>
                    ...
                    <div className="overflow-x-auto font-roboto font-bold"><table className="w-full text-left text-[9px] divide-y divide-slate-100 uppercase font-mono font-bold font-roboto">
                        ...
                        <tbody className="divide-y text-slate-600 font-bold uppercase font-roboto">
                            {[...givenHistory, ...takenHistory].sort((a,b)=>Number(b.timestamp)-Number(a.timestamp)).map(h => (
                                <tr key={h.id} className="hover:bg-slate-50 transition border-b uppercase font-bold font-roboto">
                                    <td className="p-4 leading-tight uppercase font-bold font-roboto">
                                      <p className="text-slate-800 font-bold text-[12.5px] tracking-tight uppercase font-roboto">{h.item_name}</p>
                                      <p className="text-[8.5px] text-slate-400 mt-1.5 uppercase font-roboto">SPEC: {h.item_spec}</p>
                                    </td>
                                    <td className="p-4 text-center font-black whitespace-nowrap uppercase font-roboto">
                                      <div className="flex flex-col items-center gap-1 leading-none uppercase font-roboto">
                                        <span className="text-[9.5px] text-blue-600/80 font-bold uppercase tracking-tighter font-roboto">UDH: {h.req_qty} {h.item_unit}</span>
                                        <span className={`text-[9.5px] font-black uppercase tracking-tighter font-roboto ${h.status === 'returned' ? 'text-green-600' : 'text-slate-300'}`}>RET: {h.status === 'returned' ? h.req_qty : 0} {h.item_unit}</span>
                                      </div>
                                    </td>
                                    <td className="p-4 leading-tight uppercase font-bold font-roboto"><p className="text-blue-500 font-bold uppercase font-roboto">BORR: {h.from_name}</p><p className="text-red-500 font-bold uppercase mt-1 font-roboto">LEND: {h.to_name}</p></td>
                                    <td className="p-4 leading-none space-y-1.5 font-bold tracking-tighter uppercase text-[8.5px] font-roboto">
                                        <p><span className="opacity-50 font-bold font-roboto">1. REQUEST BY:</span> {h.from_name} ({h.from_unit}) (QTY: {h.req_qty}) @ {formatTS(h.timestamp)}</p>
                                        <p><span className="opacity-50 font-bold font-roboto">2. APPROVED BY:</span> {h.to_name} (QTY: {h.req_qty}) @ {formatTS(h.timestamp)}</p>
                                        <p><span className="opacity-50 font-bold font-roboto">3. RETURN BY:</span> {h.from_name} (QTY: {h.req_qty}) @ {formatTS(h.timestamp)}</p>
                                        <p><span className="opacity-50 font-black text-green-600 font-bold font-roboto">4. FINAL VERIFY:</span> {h.to_name} (QTY: {h.req_qty}) @ {formatTS(h.timestamp)} (LOGGED)</p>
                                    </td>
                                    <td className="p-4 text-center uppercase font-roboto"><span className={`px-2.5 py-1 rounded-full text-[8.5px] font-black uppercase tracking-widest font-roboto ${h.status==='returned' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{h.status}</span></td>
                                </tr>
                            ))}
                        </tbody></table></div>
                </div>
