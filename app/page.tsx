...
export default function SpareSetuApp() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f1f5f9]">
      {/* --- SIDEBAR (IDENTICAL LOOK) --- */}
      <aside className="w-64 bg-white hidden md:flex flex-col flex-shrink-0 z-20 shadow-xl border-r border-slate-200">
        <div className="p-6 border-b border-slate-100"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded bg-slate-100 flex items-center justify-center text-orange-600"><i className="fa-solid fa-layer-group"></i></div><span className="text-lg font-bold text-slate-800 font-industrial uppercase tracking-wide">Menu</span></div></div>
        <nav className="flex-1 px-3 space-y-1 mt-4">
          <button onClick={() => setActiveTab("search")} className={`nav-item w-full text-left flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-sm font-medium ${activeTab === 'search' ? 'active-nav' : 'text-slate-600'}`}><i className="fa-solid fa-globe w-5"></i> <span>Global Search</span></button>
          <button onClick={() => setActiveTab("mystore")} className={`nav-item w-full text-left flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-sm font-medium ${activeTab === 'mystore' ? 'active-nav' : 'text-slate-600'}`}><i className="fa-solid fa-warehouse w-5"></i> <span>My Local Store</span></button>
          <button onClick={() => setActiveTab("analysis")} className={`nav-item w-full text-left flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-sm font-medium ${activeTab === 'analysis' ? 'active-nav' : 'text-slate-600'}`}><i className="fa-solid fa-chart-simple w-5"></i> <span>Monthly Analysis</span></button>
          <button onClick={() => setActiveTab("usage")} className={`nav-item w-full text-left flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-sm font-medium ${activeTab === 'usage' ? 'active-nav' : 'text-slate-600'}`}><i className="fa-solid fa-clipboard-list w-5"></i> <span>My Usage</span></button>
          <button onClick={() => setActiveTab("returns")} className={`nav-item w-full text-left flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-sm font-medium ${activeTab === 'returns' ? 'active-nav' : 'text-slate-600'}`}><i className="fa-solid fa-hand-holding-hand w-5"></i> <span>Returns & Udhaari</span></button>
        </nav>
        <div className="p-4 border-t border-slate-100 bg-slate-50">
          <div className="flex items-center gap-3 p-3 rounded-lg bg-white border mb-2 shadow-sm"><div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-white font-bold text-xs">{profile?.name?.charAt(0)}</div><div className="overflow-hidden"><p className="text-[9px] text-slate-400 font-bold uppercase">Logged in</p><div className="text-xs font-bold text-slate-700 truncate">{profile?.name}</div></div></div>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto relative pb-20 md:pb-0">
        ...
        <header className="header-bg text-white sticky top-0 z-30 shadow-md">
          <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <h2 className="font-industrial text-2xl text-orange-500 tracking-[0.1em] font-bold uppercase hidden md:block">SPARE SETU PORTAL</h2>
          </div>
        </header>
...
function AuthView() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 login-bg">
      {view === "login" && (
        <div className="w-full max-w-md login-card rounded-2xl shadow-2xl p-8 fade-in border-t-4 border-orange-500 relative">
          ...
          <div className="space-y-4">
            ...
            <div className="relative"><i className="fa-solid fa-lock absolute left-4 top-3.5 text-slate-400"></i><input type="password" value={form.pass} className="w-full pl-10 pr-4 py-3 rounded-lg login-input outline-none text-sm" placeholder="Password" onChange={e=>setForm({...form, pass:e.target.value})} /></div>
            <div className="text-right"><button onClick={()=>alert("Reset logic check email link")} className="text-xs text-orange-500 hover:text-orange-400 font-bold transition">Forgot Password?</button></div>
          </div>
          <button onClick={handleAuth} className="w-full h-12 mt-6 iocl-btn text-white font-bold rounded-lg shadow-lg flex items-center justify-center gap-2">Secure Login <i className="fa-solid fa-arrow-right"></i></button>
          <div className="mt-6 text-center border-t border-white/10 pt-4"><p className="text-xs text-slate-400">New User? <button onClick={()=>setView('register')} className="text-white hover:text-orange-500 font-bold underline ml-1">Create Account</button></p></div>
      ...
      {view === "register" && (
        <div className="w-full max-w-md login-card rounded-2xl shadow-2xl overflow-hidden relative z-10 fade-in border-t-4 border-orange-500">
            <div className="p-8 pb-2 text-center">
                <div className="flex justify-center mb-4"><div className="iocl-logo-container" style={{ fontSize: '12px' }}><div className="iocl-circle"><div className="iocl-band"><span className="iocl-hindi-text">इंडियनऑयल</span></div></div></div></div>
                <h1 className="font-industrial text-2xl font-bold text-white uppercase tracking-wider">Register</h1>
            </div>
            <div className="p-8 pt-4 space-y-4">
                ...
                <div className="relative"><i className="fa-solid fa-building absolute left-4 top-3.5 text-slate-400"></i>
                    <select className="w-full pl-10 pr-4 py-3 rounded-lg login-input outline-none text-sm appearance-none text-slate-300 bg-slate-900" onChange={e=>setForm({...form, unit:e.target.value})}>
                ...
                <div className="relative"><i className="fa-solid fa-envelope absolute left-4 top-3.5 text-slate-400"></i><input type="email" placeholder="Official Email" className="w-full pl-10 pr-10 py-3 rounded-lg login-input outline-none text-sm" onChange={e=>setForm({...form, email:e.target.value})} /></div>
                <div className="relative"><i className="fa-solid fa-lock absolute left-4 top-3.5 text-slate-400"></i><input type="password" placeholder="Create Password" className="w-full pl-10 pr-4 py-3 rounded-lg login-input outline-none text-sm" onChange={e=>setForm({...form, pass:e.target.value})} /></div>
                <button onClick={handleAuth} className="w-full h-12 mt-4 iocl-btn text-white font-bold rounded-lg shadow-lg">Register Now</button>
                <div className="mt-6 text-center border-t border-white/10 pt-4"><p className="text-xs text-slate-400">Already have an account? <button onClick={()=>setView('login')} className="text-white hover:text-orange-500 font-bold underline ml-1">Back to Login</button></p></div>
            </div>
        </div>
      )}

      {view === "otp" && (
        <div className="w-full max-w-md login-card rounded-2xl shadow-2xl p-8 fade-in border-t-4 border-orange-500 text-center">
            <h2 className="text-2xl font-bold text-white mb-2">Enter OTP</h2>
            <p className="text-slate-400 text-sm mb-6">We sent a code to {form.email}</p>
            <input type="text" value={form.enteredOtp} maxLength={6} className="w-full text-center text-3xl tracking-[0.5em] font-bold py-4 rounded-lg login-input outline-none mb-6" placeholder="######" onChange={e=>setForm({...form, enteredOtp:e.target.value})} />
            <button onClick={handleAuth} className="w-full h-12 iocl-btn text-white font-bold rounded-lg shadow-lg uppercase">Verify Access</button>
        </div>
      )}
    </div>
  );
...
// --- GLOBAL SEARCH (TOTAL STOCK + BREAKDOWN) ---
function GlobalSearchView({ profile }: any) {
  ...
  const filtered = Object.values(grouped).filter((i: any) => {
    const mSearch = i.item.toLowerCase().includes(search.toLowerCase()) || i.spec.toLowerCase().includes(search.toLowerCase());
    const mCat = selCat === "all" ? true : (selCat === "zero" ? i.totalQty === 0 : i.cat === selCat);
    const mSub = selSub === "all" ? true : i.sub === selSub;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <section className="bg-white p-4 rounded-xl border flex flex-wrap items-center gap-4 shadow-sm">
        ...
        <div className="flex gap-3 overflow-x-auto flex-1">{contributors.map((c, idx) => (<div key={idx} className="bg-slate-50 p-2 rounded-lg border flex items-center gap-3 min-w-[180px] shadow-sm"><div className="w-8 h-8 rounded-full bg-slate-800 text-white flex items-center justify-center font-bold text-xs border-2 border-orange-400">{c.name.charAt(0)}</div><div><p className="text-xs font-bold text-slate-800">{c.name}</p><p className="text-[9px] text-slate-400">{c.unit}</p><p className="text-[9px] font-bold text-green-600">{c.item_count || 0} Items</p></div></div>))}</div>
      </section>

      <section className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b bg-slate-50/80 flex flex-wrap items-center gap-2">
             <div className="relative flex-grow md:w-80"><i className="fa-solid fa-search absolute left-3 top-3 text-slate-400"></i><input type="text" placeholder="Global Inventory Search..." className="w-full pl-9 pr-4 py-2 border rounded-md text-sm outline-none" onChange={e=>setSearch(e.target.value)} /></div>
             ...
             <button onClick={()=>setShowSummary(true)} className="bg-indigo-600 text-white px-3 py-2 rounded-md text-xs font-bold ml-auto flex items-center gap-2"><i className="fa-solid fa-chart-pie"></i> Summary</button>
             <button onClick={()=>alert("Exporting Global CSV...")} className="bg-emerald-600 text-white px-3 py-2 rounded-md text-xs font-bold flex items-center gap-2"><i className="fa-solid fa-file-csv"></i> Export</button>
        </div>
      ...
      {showSummary && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-scale-in max-h-[80vh] flex flex-col">
                <div className="bg-indigo-50 px-6 py-4 border-b flex justify-between items-center"><h3 className="text-lg font-bold text-indigo-900 uppercase">Stock Summary (All Zones)</h3><button onClick={()=>setShowSummary(false)} className="text-slate-400">✕</button></div>
                <div className="overflow-y-auto p-4">
                    {[...new Set(items.map(i=>i.cat))].sort().map(cat => (
                        <div key={cat} className="border rounded-xl mb-4 overflow-hidden shadow-sm">
                            <div className="bg-slate-100 p-2 text-[10px] font-black text-slate-600 uppercase tracking-widest">{cat}</div>
                            <table className="w-full text-sm"><tbody className="divide-y">
                                {[...new Set(items.filter(i=>i.cat===cat).map(i=>i.sub))].sort().map(sub => (<tr key={sub}><td className="p-3 pl-6 text-slate-600">{sub}</td><td className="p-3 text-right pr-6 font-bold text-indigo-600">{items.filter(i=>i.cat===cat && i.sub===sub).reduce((sum, item)=>sum+item.qty, 0)} Nos</td></tr>))}
                            </tbody></table>
                        </div>
                    ))}
                </div>
      ...
      {breakdown && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-2xl rounded-2xl p-6 relative shadow-2xl animate-scale-in">
             <button onClick={()=>setBreakdown(null)} className="absolute top-4 right-4 text-slate-400">✕</button>
             ...
             <div className="overflow-hidden border border-slate-100 rounded-xl bg-slate-50/50">
               <table className="w-full text-left"><thead className="bg-slate-100 text-xs font-bold text-slate-500 uppercase border-b"><tr><th className="p-4">Unit / Zone</th><th className="p-4">Engineer</th><th className="p-4 text-right">Qty</th><th className="p-4 text-center">Action</th></tr></thead>
                 <tbody className="divide-y text-sm bg-white">
                   {breakdown.holders.map((h:any, idx:number)=>(<tr key={idx} className="hover:bg-indigo-50/30 transition"><td className="p-4 font-bold text-slate-700">{h.holder_unit}</td><td className="p-4 text-slate-500">{h.holder_name}</td><td className="p-4 text-right font-bold text-indigo-600">{h.qty} Nos</td><td className="p-4 text-center">{h.holder_uid === profile?.id ? <span className="text-[10px] text-green-500 font-bold uppercase italic">Your Stock</span> : <button className="bg-orange-500 text-white px-3 py-1 rounded text-xs font-bold shadow-sm" onClick={()=>alert(`Request sent to ${h.holder_name}`)}>Request</button>}</td></tr>))}
                 </tbody>
               </table>
             </div>
          </div>
...
// --- MY LOCAL STORE (IDENTICAL ALERT + ICONS) ---
function MyStoreView({ profile, fetchProfile }: any) {
  ...
  const handleSave = async () => {
    ...
    if (!error) { 
        ...
        await supabase.from("profiles").update({ item_count: (profile.item_count || 0) + 1 }).eq('id', profile.id); fetchProfile();
    ...
    <div className="animate-fade-in space-y-6 pb-10">
      ...
      {outOfStockCount > 0 && (
        <div className="bg-red-50 border border-red-200 p-4 rounded-xl flex items-start gap-3 animate-fade-in shadow-sm border-l-8 border-l-red-500">
          ...
          <div><h3 className="font-bold text-red-800 text-sm uppercase">Action Needed: Restock Required</h3><p className="text-xs text-red-600 font-bold">{outOfStockCount} items are out of stock in your zone.</p></div>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-6 rounded-xl border shadow-sm">
        ...
        <div className="flex gap-2">
            <button onClick={()=>alert("Exporting...")} className="bg-emerald-600 text-white px-4 py-2.5 rounded-xl font-bold text-xs shadow-md flex items-center gap-2"><i className="fa-solid fa-file-csv"></i> Export CSV</button>
            <button onClick={() => setShowAddModal(true)} className="iocl-btn text-white px-6 py-2.5 rounded-xl font-bold shadow-md flex items-center gap-2 transition hover:scale-105"><i className="fa-solid fa-plus"></i> Add New Stock</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b bg-slate-50/80 flex flex-wrap items-center gap-2">
             ...
             <select className="border rounded-md text-[11px] font-bold p-2 bg-white" onChange={e=>setSelCat(e.target.value)}><option value="all">Category: All</option>{[...new Set(myItems.map(i => i.cat))].sort().map(c => <option key={c} value={c}>{c}</option>)}</select>
        </div>
        <div className="overflow-x-auto"><table className="w-full text-left">
          ...
          <tbody className="divide-y text-sm">
            {filtered.map(i => (
              <tr key={i.id} className={`${Number(i.qty) === 0 ? 'bg-red-50/30' : 'hover:bg-slate-50'} transition border-b border-slate-50`}>
                ...
                <td className="p-5"><span className="bg-slate-100 border px-2 py-1 rounded text-[11px] font-medium text-slate-600">{i.spec}</span></td>
                <td className="p-5 font-bold text-center">{Number(i.qty) === 0 ? <span className="text-red-600 font-black text-[10px] uppercase bg-red-100 px-2 py-1 rounded border border-red-200">Out of Stock</span> : <span className="text-emerald-600 text-lg font-black">{i.qty} Nos</span>}</td>
                <td className="p-5 flex gap-3 justify-center items-center">
                    <button onClick={()=>setConsumeItem(i)} disabled={Number(i.qty) === 0} className="text-indigo-600 hover:scale-125 transition disabled:opacity-30" title="Consume"><i className="fa-solid fa-box-open text-xl"></i></button>
                    <button onClick={()=>setEditItem(i)} className="text-slate-400 hover:text-blue-500 hover:scale-125 transition" title="Edit"><i className="fa-solid fa-pen-to-square text-xl"></i></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {consumeItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-scale-in">
            <div className="bg-indigo-600 px-6 py-4 border-b flex justify-between items-center text-white font-bold uppercase"><h3 className="text-lg">Consume Stock</h3><button onClick={()=>setConsumeItem(null)}>✕</button></div>
            <div className="p-6 space-y-4 text-center">
                <div className="text-sm font-bold text-slate-700">{consumeItem.item}</div>
                <div className="bg-slate-50 p-4 rounded-xl border border-dashed border-slate-300"><p className="text-[10px] font-black text-slate-400 uppercase">Available</p><p className="text-3xl font-black text-emerald-600">{consumeItem.qty} Nos</p></div>
                <input type="number" id="cQty" placeholder="Qty to remove" className="w-full p-4 border-2 border-slate-200 rounded-xl text-2xl font-black text-center focus:border-indigo-500 outline-none" />
                <button onClick={async ()=>{
                    const qInput = (document.getElementById('cQty') as HTMLInputElement).value; const qUsed = parseInt(qInput);
                    if(!qUsed || qUsed > consumeItem.qty) return alert("Invalid Quantity!");
                    const {error} = await supabase.from('inventory').update({qty: consumeItem.qty - qUsed}).eq('id', consumeItem.id);
                    if(!error){
                        await supabase.from('usage_logs').insert([{ item_name: consumeItem.item, category: consumeItem.cat, spec: consumeItem.spec, qty_consumed: qUsed, consumer_uid: profile.id, consumer_name: profile.name, consumer_unit: profile.unit }]);
                        alert("Done!"); fetch(); setConsumeItem(null);
                    }
                }} className="w-full py-4 bg-indigo-600 text-white font-black rounded-xl shadow-lg uppercase tracking-widest hover:bg-indigo-700">Confirm Job Done</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md overflow-y-auto">
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-8 relative animate-scale-in my-auto">
            <button onClick={() => setShowAddModal(false)} className="absolute top-4 right-4 text-slate-400 font-bold text-xl transition">✕</button>
            ...
            <div className="flex items-center justify-between bg-yellow-50 p-3 rounded-xl border border-yellow-200 mb-6">
                <span className="text-xs font-bold text-yellow-800 uppercase"><i className="fa-solid fa-circle-info mr-1"></i> Item not in list?</span>
            ...
            <div className="space-y-4">
              {!form.isManual ? (<>
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Category</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-slate-50" value={form.cat} onChange={e=>setForm({...form, cat: e.target.value, sub:"", make:"", model:"", spec:""})}><option value="">-- Select --</option>{categories.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Sub Category</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-white" disabled={!form.cat} value={form.sub} onChange={e=>setForm({...form, sub: e.target.value, make:"", model:"", spec:""})}><option value="">-- Select --</option>{subs.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Make</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-white" disabled={!form.sub} value={form.make} onChange={e=>setForm({...form, make: e.target.value, model:"", spec:""})}><option value="">-- Select --</option>{makes.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                </div>
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Model</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-white" disabled={!form.make} value={form.model} onChange={e=>setForm({...form, model: e.target.value, spec:""})}><option value="">-- Select --</option>{models.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Spec</label><select className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm bg-white" disabled={!form.model} value={form.spec} onChange={e=>setForm({...form, spec: e.target.value})}><option value="">-- Select --</option>{specs.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
              </>) : (<div className="space-y-4">
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Item Name</label><input type="text" placeholder="Material Name" className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm" onChange={e=>setForm({...form, model: e.target.value})} /></div>
                    <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Technical Spec</label><input type="text" placeholder="Details..." className="w-full p-3 border-2 border-slate-100 rounded-lg text-sm" onChange={e=>setForm({...form, spec: e.target.value})} /></div>
              </div>)}
...
function UsageHistoryView({ profile }: any) {
  ...
  return (<section className="bg-white rounded-xl border border-slate-200 overflow-hidden"><div className="p-5 border-b bg-slate-50/50 flex justify-between items-center"><h2 className="text-lg font-bold text-slate-800 font-industrial uppercase">Consumption Logs</h2></div><div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-slate-50 text-slate-500 text-[10px] font-bold border-b uppercase"><tr><th className="p-5">Date</th><th className="p-5">Details</th><th className="p-5 text-center">Qty</th></tr></thead><tbody className="divide-y text-sm">{logs.map(l => (<tr key={l.id} className="hover:bg-slate-50 transition border-b border-slate-50"><td className="p-5 text-xs text-slate-500 font-bold leading-tight">{new Date(Number(l.timestamp)).toLocaleDateString()}</td><td className="p-5 font-bold text-slate-800">{l.item_name}<div className="text-[10px] text-slate-400 uppercase mt-0.5">{l.category}</div></td><td className="p-5 text-center font-black text-red-600">-{l.qty_consumed} Nos</td></tr>))}</tbody></table></div>{logs.length===0 && <div className="p-20 text-center italic text-slate-400">No records.</div>}</section>);
}

function MonthlyAnalysisView({ profile }: any) {
  const [analysis, setAnalysis] = useState<any[]>([]);
  useEffect(() => { const f = async () => { const { data } = await supabase.from("usage_logs").select("*").eq("consumer_uid", profile.id); if (data) { const stats: any = {}; data.forEach((l: any) => { const m = new Date(Number(l.timestamp)).toLocaleString('default', { month: 'long', year: 'numeric' }); if (!stats[m]) stats[m] = { total: 0, count: 0 }; stats[m].total += Number(l.qty_consumed); stats[m].count += 1; }); setAnalysis(Object.entries(stats).map(([m, d]: any) => ({ month: m, ...d }))); } }; if (profile) f(); }, [profile]);
  return (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">{analysis.map((a, idx) => (<div key={idx} className="bg-white p-6 rounded-2xl border shadow-sm text-center transition hover:shadow-md"><div className="text-xs font-black text-slate-400 uppercase mb-4 tracking-widest">{a.month}</div><div className="w-16 h-16 bg-blue-50 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl shadow-inner"><i className="fa-solid fa-chart-line"></i></div><div className="text-3xl font-black text-slate-800">{a.total} <small className="text-[10px] text-slate-400 font-bold uppercase">Nos</small></div><div className="text-[10px] font-bold text-emerald-500 mt-2 uppercase">{a.count} Consumptions</div></div>))}{analysis.length===0 && <div className="p-20 text-center italic text-slate-400 bg-white rounded-xl border w-full col-span-3">No monthly data.</div>}</div>);
}

function ReturnsLedgerView({ profile }: any) { return (<div className="bg-white p-20 rounded-xl border text-center italic text-slate-400 border-dashed uppercase tracking-widest text-xs font-black">Returns & Udhaari logic attached to Requests table.</div>); }